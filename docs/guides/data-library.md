...
- Import copies each accepted original into managed storage while hashing it, verifies deduplicated originals before trusting them, and keeps originals and sidecars. Interpretation identity is content plus interpretation facts, never names, so identical bytes with different meanings stay distinct.
...
- A published item refuses new sources ("already published and fixed"). Publication is a short catalogue transaction that advances the item's head.
...
- Restart recovery marks staging, applying and preparing jobs failed ("interrupted by restart"); published data is unaffected, and nothing is recalculated because the app reopened.
...
- Every LiDAR command is executor-backed (`NativeOperationClass::UserData`) except the reviewed bounded cancel signals `lidar_cancel_import`, `lidar_cancel_analysis_job` and `lidar_cancel_sample_pixel` in `desktop/src/native_command_policy.rs`. Import, slope and display preparation jobs run on the `Local` class. Rules are in [Native and release](native-and-release.md).
- `engine.rs` runs the GDAL CLI tools (`gdalinfo`, `gdal_translate`, `gdaltransform`) with fixed argv, no shell, bounded output and duration, cancellation, and a cached, recorded engine version. Discovery: `CANOPI_LIDAR_GDAL_BIN` directory, then `PATH`. The COG options need GDAL 3.7+ (validated on 3.8.4).
