...
- `commands/registry.ts` is the public App Command Graph seam. The catalog, projections and shortcut adapter live in `commands/graph/`, which callers never import directly. `MenuBar`, `CommandPalette`, the panel rail, the tool rail and `shortcuts/manager.ts` all consume the registry and never duplicate actions, availability, active state or shortcut maps.
- Menus come from one model: `app/shell-commands/menus.ts` (`composeWorkspaceMenus`) builds File, Edit, View, Tools and Help from the shell descriptors' menu placements plus the canvas projection (tools, history, edits, view actions, Grid/Snap/Rulers). Checkable entries render as `menuitemcheckbox` or `menuitemradio`, and the Keyboard shortcuts dialog (F1) lists the same menus. Shell capabilities that are not panels or files (Rename F2, Settings Ctrl ,, Find plants, Tool names, map background, theme, shortcuts, About) live in `app/workspace-commands/capabilities.ts`; canvas intents for both editions in `app/workspace-commands/canvas-actions.ts`.
- Shortcut strings are canonical (`Ctrl+Shift+Z`, `Ctrl+Plus`); `app/shell-commands/shortcut-text.ts` formats them with localized key names (`shortcutKeys.*`) and derives `aria-keyshortcuts` with both Control and Meta. Web drops shortcuts a browser keeps for itself.
...
- Ctrl F focuses the open panel's plant finder (`app/plant-finder/focus.ts`, routed before the command graph by `shortcuts/manager.ts` and `web/canvas-shortcuts.ts`); place search is Ctrl K (`view.searchPlace`), which focuses the title-bar place field. Edit › Find plants (Ctrl F) opens the Plant Database when no finder is mounted.
...
- The map is full-bleed; chrome floats over it. `components/shared/WorkspaceTitleBar.tsx` (Desktop wraps it in `TitleBar.tsx` with window controls) holds the menubar, the Design name button (rename, F2), `SaveStatusLabel`, the place field (Ctrl K), Help (F1) and Settings (Ctrl ,). Below 760 px the menubar collapses to one Menu button with inline submenus. Web shows "Saved in this browser", Download a copy and an Open `.canopi` icon.
- `components/canvas/CanvasChrome.tsx` composes the tool rail (`ToolRail.tsx`), the view chip (`ViewChip.tsx`: Grid, Snap, Rulers), the zoom group (`ZoomControls.tsx`: scale bar, −, 1:N menu, +, Fit) with the attribution left of it, and the overview chip. Edition-only overlays enter through its children slot.
- The tool rail shows labels and keys until each rail tool has been used once on this device (`app/tool-rail/learning.ts`, settings `used_canvas_tools`); View › Tool names (`tool_names_visible`) overrides it. The platform bootstraps install the learning listener.
- Settings (`SettingsDialog.tsx`) and Keyboard shortcuts (`KeyboardShortcutsDialog.tsx`) open through `app/shell/dialogs.ts` and share `WorkspaceDialog.tsx` (modal, focus trap, Esc returns focus).
...
- Panels from `app/shell-commands/index.ts`:
  ...
  - **design:** Layers, Data, Plants in this Design (`species-key`), Plant Database, Favorites, Design Notebook (Desktop).
  - **planning:** Calendar, Budget, Consortium.
  - Ctrl 1–8: Layers, Plants in this Design, Plant Database, Favorites, Calendar, Budget, Consortium, Design Notebook. Data has no key.
- Only one side panel is open at a time, in a floating dock (`SidePanelDock`) left of the panel rail: 380 px, or 440 px for Budget and Consortium. The dock starts closed. Design-dependent entries are disabled with no Design, except an open entry, which can still close. There is no Location panel: place search is the title-bar place field (see [map workspace](map-workspace.md)).
