...
- The shared `Settings` contract lives in `common-types/src/settings.rs`: `locale` (11 languages), `theme` (`light` or `dark`), snapping, `auto_save_interval_s`, panel sizes, `plant_spacing_interval_m`, map layers (`basemap_style`, `satellite_provider`, visibility and opacity for basemap, satellite, contours and hillshade, `contour_interval`), `google_maps_api_key` (a device-local credential), and `last_view { lon, lat, zoom }`. Unknown keys are ignored and never re-emitted, and no value is rewritten.
