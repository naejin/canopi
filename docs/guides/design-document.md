...
- Web homes are always browser Drafts (`web/browser-app-data.ts`); an opened `.canopi` becomes a new Draft. A download is an export and never changes the home. On start with no active Design, the Web platform bootstrap (`platform/browser.ts`) restores the newest valid Draft before first render, then installs continuous save for the application lifetime (Desktop installs it in `platform/desktop.ts`). Each Draft body is decoded on its own, so one corrupt Draft does not discard the others. The Start screen (`components/shared/StartScreen.tsx`, both editions) lists Drafts with Open and Delete (inline confirmation); Desktop also lists Recent Designs. New Design then asks "Where is your site?" (`app/site-onboarding/state.ts`, `components/canvas/SiteOnboarding.tsx`) and offers "Start your Design" beside the labelled tool rail.
...
- Status is `saved`, `saving`, `draft`, `error` or `conflict`. The title bar shows it beside the Design name (`components/shared/SaveStatusLabel.tsx`): "Saving…", "Saved", "Draft" with Save as…, "Couldn't save" with Details… (a short reason from `failureReason`, Retry and Save as…), or "Changed outside Canopi" with Resolve…, which opens the conflict dialog. Raw error details go to the log, never the UI. A dirty Design without a home reports `error`.
