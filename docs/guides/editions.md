...
- The Browser App Shell (`web/BrowserAppShell.tsx`, fed by `web/browser-shell-commands.ts`) offers New, Open `.canopi`, Download `.canopi`, Export to PDF, GeoJSON import and export, theme and panel toggles. There are no native window controls, Recent Designs, reveal actions, Design Notebook, updater or Problem Report. The welcome screen omits recent files.
- Browser-local data lives in `web/browser-app-data.ts`: four independent localStorage records (Drafts, Settings, Species activity, Saved Object Stamps). A missing, corrupt or foreign record reads as empty, and a failed write returns `{ ok: false }` and keeps the previous record. v1 storage is ignored. Drafts are the Web Design home: continuous save writes them and the welcome screen lists them. A downloaded `.canopi` is an export.
