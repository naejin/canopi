...
- There is one schema and no migrations. `UserDb::open` renames an older DB to `<file>.v<version>-set-aside` and a non-SQLite, damaged or foreign-key-violating one to `<file>.corrupt-<unix-seconds>`, moving its `-journal` (and any `-wal`/`-shm`) companions with it and never overwriting an earlier set-aside file, then starts empty. It refuses a newer DB with `UserDbInitError::NewerSchemaVersion` and leaves it in place. A schema change edits the one file and bumps the version.
