...
Production should keep shared generation separate from compile-time edition delivery adapters. The current native PDF export command accepts a PNG snapshot and a native layout; it cannot deliver these shared PDF bytes unchanged. A native adapter will need a save dialog and executor-backed binary write, with cancellation and I/O errors kept separate from Design save state. The browser adapter can use the tested Blob/download mechanism, subject to browser delivery tests. No new native command was added here.

Linux WebKitGTK, macOS WKWebView, Windows WebView2, Firefox, Safari, physical printers, offline/slow-network acquisition, low-memory devices, and large Design cancellation were **not** exercised. Unicode line breaking in the fixture is deliberately limited; complete wrapping, unsupported text, fonts and resource lifetime must be verified in the implementation slices. These obligations remain in `canopi-w4p3`, `canopi-urxb`, `canopi-h0q3` and `canopi-1zbj`; this evaluation does not close the cross-platform release gates.
