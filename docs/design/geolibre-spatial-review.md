...
## 9. GPT-5.6 Sol execution guide: reuse first, minimize total cost

### Role and operating policy

Sol is the lead implementer/orchestrator and integration owner. It keeps the domain decisions, dependency order, review findings and final evidence coherent. Use Terra for implementation against settled interfaces and Luna for clear repeatable tasks. These assignments are engineering recommendations, not measured Canopi-specific model success rates. Do not promise a cost reduction before observing completed work.

| Model | Default assignment | Escalation boundary |
| --- | --- | --- |
| Sol | GeoLibre reuse assessment, spatial/frame contracts, graphics integration, undo/settlement decisions, difficult failures, integrated review | Work locally when delegation would repeat the same reasoning or add coordination without useful independent work. |
| Terra | Bounded renderer adapters after feasibility, typed wiring, incremental layer reconciliation, substantive regression tests | Missing contract, cross-authority redesign, graphics-state uncertainty or repeated failed repair goes to Sol. |
| Luna | Fixture receipts, deterministic scripts, approved repetitive updates, narrow tests with independently specified expectations, documentation | No autonomous camera architecture, shared graphics context, persistence transaction or complex gesture ownership work. |

Start at medium reasoning for routine workers; use high for Sol's consequential reviews/decisions and Terra's complex but bounded changes. Higher effort is a response to demonstrated difficulty, not the default for every task. Model/effort settings are recommendations; respect the available model catalogue and tool capabilities at execution time.

Codex supports model-configured subagents without an additional plugin; see [official subagent guidance](https://learn.chatgpt.com/docs/agent-configuration/subagents). In this environment, model overrides require a compact/fresh fork rather than inheriting the full conversation. Inspect current spawn-tool constraints instead of assuming this stays identical across clients. Never silently substitute the lead model for a requested cheap worker and claim cheap-model results.

Start with **one implementation worker**, while Sol does independent review, interface refinement or verification. Use additional workers only for truly independent bounded work. Dependency availability does not establish safe concurrent writes; keep one writer per file/module. Workers do not delegate further unless that delegation is explicitly authorized and useful. Do not run an idle lead in a verbose polling loop.

### First implementation session

1. Read `git status`, this plan's fixed decisions and existing LiDAR bead summaries. Preserve dirty/untracked planning files and accepted code. Inspect the integrated branch before creating a checkout. No reset, stash or deletion of user work as setup.
2. Read only the guides for the next slice. Resolve current symbols with `rg`; do not reload all docs or historical conversation. Treat old measurement reports as evidence with their stated limits, not current performance facts.
3. Record a pinned GeoLibre reuse inventory and installed Canopi MapLibre/Pixi versions. Map each required behavior to existing Canopi code, a reusable upstream module or missing functionality. Do this before inventing new adapters.
4. When implementation has been explicitly started, create a fresh epic and only the near-term executable tasks: reuse/contract decision, fixture/baseline tooling and bounded rendering feasibility. The deleted eight-bead graph must not be recreated by rote. Keep later phases as outcomes until interfaces are proven.
5. Prepare the private representative fixture and matched baseline. Freeze spatial/frame/graphics interfaces only to the extent required for the next experiment. Test representative plant appearance and shared context early.
6. At Phase B exit, Sol reviews the actual code and evidence, chooses the qualified adapter, then writes precise downstream task packets. If the approach fails, report the failed requirement before funding broad downstream implementation.

### GeoLibre reuse inventory

The earlier checkout was `/tmp/canopi-geolibre-review` at the revision in section 8; it may no longer exist. Retrieve a read-only pinned checkout outside Canopi if necessary. Do not run upstream install scripts or introduce its complete dependency tree merely to browse code. Consult current official APIs only where the installed version or local source leaves a gap.

| Upstream starting point | Investigate for reuse | Canopi constraint |
| --- | --- | --- |
| `packages/map/src/layer-sync.ts` | Stable source IDs, incremental data/style updates, ordering, reload reconstruction | Extract cohesive behavior and tests; remove store/framework coupling rather than mirror another layer authority. |
| `packages/map/src/map-controller.ts`, `MapCanvas.tsx` | Camera command handling, gesture interruptions and map lifecycle | Live frame observation must serve Canopi tools; persisted `moveend` state is not its render clock. |
| `packages/plugins/src/plugins/raster-layer-sync.ts` | Raster/native/custom layer ordering and state transitions | Keep Canopi's managed library, generation publication and independent source/result presentation. Avoid importing bidirectional control/store state. |
| `packages/plugins/src/plugins/geo-editor-geometry.ts` | Pure geometry and feature-identity helpers | Keep SceneStore IDs, tools, locks and history authoritative. No replacement with a generic GIS editor. |
| `packages/map/src/map-resize.ts` | Single lifetime owner for resize work | Do not delay only one surface while another uses new dimensions. |
| `packages/processing/src/types.ts` | Declarative analysis parameters/capabilities | Later analysis UI only if already in scope; not permission to add a broad toolbox or replace the native engine. |

For each candidate record: pinned file/symbol, dependencies, exact behavior needed, caller interface, copy/adapt/reimplement/reject choice, retained tests, maintenance owner and upstream update strategy. Cite relevant upstream fixes/tests rather than copying unexplained workarounds.

Prefer an intact cohesive module when it fits. Prefer a small Canopi-native implementation when extraction would retain extensive React/Zustand/plugin coupling or require a second authority. Shared render lifecycle and botanical editing remain things to prove, not capabilities assumed from upstream screenshots. Retain required notices for copied code. New runtime dependencies need concrete justification in the bead/PR.

### Compact task packet for every worker

Each executable bead must be understandable without the full epic transcript. Use its description/design/acceptance fields, not separate task Markdown files. Include:

```text
Outcome: one observable result and a concrete before/after example.
Prerequisites: integrated commits and exact interface decisions this task uses.
Read first: relevant plan sections, guide and a few existing symbols/tests.
Own: explicit allowed edit surfaces; identify shared files requiring serialization.
Contract: input/output types, owner, units, ordering, lifecycle and error behavior.
Procedure: short implementation sequence; identify what already exists to reuse.
Non-goals: adjacent work that must not enter this diff.
Acceptance: observable normal, boundary, cancellation/failure cases.
Validation: exact focused commands and applicable repository gates.
Escalate when: missing contract, repeated failure or required scope expansion.
Return: files, interface changes, tests/results, limitations and integration receipt.
```

Sol must resolve major missing contracts before assigning a cheap worker. Do not ask a worker to both invent cross-cutting semantics and implement them from a broad directory list. Do not fragment work into trivial passes that repeatedly reread the same files; each task should produce one reviewable behavior. Split a task when it spans several independent authorities or cannot be validated within one coherent change.

Specify meaningful test expectations before implementation for spatial, persistence and ownership boundaries. The lead reviews those expectations; the worker can implement the tests. Never let a self-consistent pair of faulty code and implementation-shaped tests certify correctness. Real-renderer checks remain necessary for graphics and timing.

Use a fresh short worker context containing the packet and necessary references. Keep stable contract text unchanged when possible, but do not assume prompt caching or a particular billing mechanism. Allow targeted exploration when source contradicts the packet; require the worker to report the contradiction instead of following stale instructions blindly.

### Cost control and escalation

- Optimize **total cost per accepted change**: lead reasoning, worker attempts, review, repairs and integration. Cheap token rates alone do not establish a cheaper result.
- Start by evaluating a few appropriate Luna/Terra tasks. Compare actual accepted outcomes and repair effort, not unrelated task token totals. Do not pay both models to implement every task just to manufacture a comparison.
- After two unsuccessful targeted attempts at the same failure, stop that retry loop. Send Sol the minimal reproducer, attempted fixes and relevant logs. Sol chooses a corrected packet, a stronger-model repair or a separately scoped blocker. Do not repeatedly regenerate the patch.
- Use deterministic scripts for repeated transformations, generated bindings, fixtures and structural checks. Keep scripts reusable and tested where failure could corrupt data. The model should not redo a solved mechanical process each session.
- Store verbose command output in temporary/local artifacts; return concise errors and result summaries. Do not repeatedly print huge files, whole bead histories, generated artifacts or complete test logs into the lead context.
- Treat successful checks as evidence tied to a specific code revision. Run focused checks while editing, required gates at their mandated boundaries, and rerun only affected checks after new changes. Mandatory AGENTS gates still apply; no cost policy exempts a bead from them.
- Keep one coherent integrated baseline and bounded branches. Avoid speculative parallel implementations of the same module, repeated merge repair and testing mismatched commit combinations. Use isolated profiles for native/browser data, not unnecessary duplicated code trees.
- Review high-risk contracts, projection, persistence, async lifetime and fallback deeply. Routine mechanical changes need focused diff/automated validation, not several redundant full-model reviews. Sol owns final integration acceptance.
- Do not elaborate every downstream phase before the rendering decision. Do not add broader GIS tools, new scientific algorithms or a general plugin framework to justify reused code.

Record cost evidence in bead notes, not a new tracker: model/effort, attempts, available usage figures, review/repair effort, commands run and acceptance outcome. If monetary usage is unavailable, say so and use measured tokens/time as labelled proxies. Do not infer subscription charges from API list prices or claim exact savings without billing evidence.

### Validation profiles for compact packets

These are abbreviations for the exact commands below; they do not override AGENTS. Run commands in the stated directory and name actual focused test files in the packet before assignment.

| Profile | Commands/evidence |
| --- | --- |
| D | Root: `git diff --check`; validate local Markdown links and referenced symbols. Docs-only: explain that code tests were skipped. |
| F | `desktop/web`: `npx tsc --noEmit`; `npx vitest run <actual-focused-files>`; full `npm test` for shared runtime changes. Add UI/edition gates where applicable. |
| UI | `desktop/web`: `npm run check:ui`, `npm run build`, `npm run build:web`; inspect relevant gallery and real host interactions. Use 11-locale parity checks already in the repository. |
| S | `desktop/web`: `npm run gen:types`, `npm run check:types`, TypeScript and focused contract tests; include Rust and persistence gates when shared contracts change. |
| R | Root: `cargo fmt --all -- --check`; `CANOPI_SKIP_BUNDLED_DB=1 cargo check --workspace`; `CANOPI_SKIP_BUNDLED_DB=1 cargo clippy --workspace --all-targets -- -D warnings`; relevant Rust tests. |
| P | Relevant F/S/R checks plus root `CANOPI_SKIP_BUNDLED_DB=1 cargo test --workspace`; exercise real session replacement/save/restore boundaries. |
| Native commands | Root: `CANOPI_SKIP_BUNDLED_DB=1 cargo test -p canopi-desktop native_command_policy::tests` when commands/executor change. |
| Real GL / performance | Section 7 and existing performance guide: actual native renderer, fixture hash, environment, matched scenarios and explicitly labelled measurement limits. Unit tests are not substitutes. |
| ALL | Applicable combined gates above after final integration/rebase, plus required native/edition/platform and representative/capacity evidence. |

If a check is unavailable, record its exact command, reason and residual risk. A
correctness, persistence, lifecycle, fallback or scientific acceptance gate
remains incomplete until satisfied. Performance reference misses remain measured
limitations under the 2026-09-16 qualification decision and do not prevent the
epic from closing once its mandatory correctness work is complete.

### Suggested implementation kickoff prompt

```text
Use GPT-5.6 Sol as lead for Canopi v2. Read
docs/design/geolibre-spatial-review.md and follow its fixed decisions.
Start implementation with the GeoLibre reuse inventory, representative fixture
baseline and bounded shared-render-lifecycle feasibility work. Create fresh
bead tracking only for the next executable slices; do not resurrect deleted drafts.
Use Terra for settled implementation and Luna for narrow repeatable tasks.
Delegate only useful independent work with compact packets and explicit ownership;
start with one worker. Sol owns high-risk decisions, review and integration.
Preserve the private 2201-plant Design and the existing LiDAR foundation.
No legacy compatibility layer. No broader GIS or analysis scope expansion.
Use the graphics checkpoint before committing to downstream renderer work.
Report measured outcomes and limitations; do not publish a release.
```

This prompt is retained as a future-session handoff template. The current
implementation is already authorized and tracked under `canopi-ltck`.
