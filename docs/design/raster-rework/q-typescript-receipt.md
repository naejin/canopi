...
Status: evidence — bounded NC1–NC2/MR1 accepted at `579880be`; Desktop completion `f9b5c10c` partial with known DB3/DB4 defects and correct saved retry cells. The user-approved [reset](qualification-reset.md) freezes harness hardening; the artifact brief is delivered and superseded by the [GeoLibre native integration assignment](geolibre-integration-agent-prompt.md). This receipt remains historical, not the native production gate. Q remains unqualified. Historical delivery claims below are revision-specific, not acceptance.
...
## Artifact decision brief after qualification reset

Read-only investigation for the [artifact decision assignment](q-engine-decision-agent-prompt.md) under the
[approved reset](qualification-reset.md). Inspected `scripts/raster-qualification/candidates.json`, the local bench
lock, the [GeoLibre reuse inventory](geolibre-reuse-inventory.md), this receipt, the standing review and the saved
run records; no registry query, download, build, experiment or fixture scan was performed. Everything below is what
those records say, not a claim about today's registry.

### 1. What exact artifact has evidence

| Item | Recorded value | Provenance |
| --- | --- | --- |
| Package | `whitebox-wasm@0.5.1` | bench lock `.rq-scratch/bench/package-lock.json` (local, gitignored) |
| Integrity | `sha512-7tN0VNDte/+AWrDX2/RJW8k4+0oPdC7BWwpccaHZyAgV7y7SR4wL0G5xoIdX09GrPwKOmhDFgUUc2fZVojNxcg==`, resolved `…/whitebox-wasm-0.5.1.tgz` | `candidates.json` artifact list; the lock agrees |
| Published module bytes as measured | `whitebox_wasm.js` sha256 `c60c6f73…` 67 083 B; `whitebox_wasm_bg.wasm` sha256 `05be6b73…` 4 396 465 B | saved run `host-build.json`, re-hashed inside the host binary during the run |
| Source correspondence | **none to the declared pin** — published 0.5.1 has `gitHead 6920ade…`, `chosenIsDescendantOfPin: false`; no published artifact has `gitHead 9c0ff4f…` | `candidates.json.sourceToArtifactCorrespondence`, recorded when the bench was installed |
| Display/CRS pairing | `cog-tiler-wasm@0.3.6`, integrity `sha512-1u94mAm…`, `gitHead` **is** the pin `a71c321…` | `candidates.json`; its runtime evidence is legacy, not re-measured after the receipt's TypeScript migration |
| Host/build identity for the saved Desktop retry | `pilot-1789907442506-21`: Linux 7.0.0-31-generic, WebKitGTK 2.52.6, Tauri 2.10.3, bundled Desktop WebView, network namespace denial with a local X11 relay | saved run directory; not committed |
| Fixture for that run | generated `plane-cog.tif`, sha256 `6aec7455…`, 16 778 048 B; 2048² Float32 `z=column-row`, two nodata holes, uncompressed tiled COG | generated by the harness; **not** a managed original |

Distinguishing the claims:

* **Independently reviewed:** the saved `fab0c381` pilot's 81 920 cells, including its two nodata cells, were
  independently checked against the specified formula, and its native/worker totals reconcile (12 reads,
  11 599 872 bytes, maximum read 1 048 576). The reviewer performed no Desktop run.
* **Implementer-reported:** the `f9b5c10c` retry (5/5 windows, 81 920/81 920 cells, 2/2 nodata, four refusal
  controls, six bundled assets, ledger reconciled, Q-LOCAL-1 and Q-HOST-1 pass) and every host/build identity
  above.
* **Old evidence, retained not re-validated:** the legacy Python probes, the `q2-local-bridge` stripped-layout
  rejection control and the pre-migration artifact/license records.
* **Unavailable provenance:** there is no local record that the measured `0.5.1` bytes were ever built from any
  inspected source revision. Its correspondence rests entirely on the recorded `gitHead` of the published package.

### 2. What decision remains

The declaration pins Whitebox source `9c0ff4f…` (whose `Cargo.toml` declares `0.6.0`), while the artifact with
evidence is the published `0.5.1` whose `gitHead` is `6920ade…`, not a descendant of the pin. The pin's own
version is unpublished, so no artifact can correspond to it today. **The mismatch is unresolved and no local
evidence resolves it:** nothing in the bench, the receipts, the saved runs or the local checkout records the pinned
source or a build from it. The published `0.5.1` is therefore the only artifact with measured transport behaviour,
and it is *not* the declared artifact. Repetition cannot fix this: every further run of the existing harness measures
the same published bytes.

### 3. Smallest next step under each user choice

**Retain the pin (`9c0ff4f…`).** The next step is one isolated pinned-source build attempt in the bench, not another
harness run. Locally inspectable prerequisites are missing: the pinned source tree is not present in this
filesystem, and the Rust toolchain has only `x86_64-unknown-linux-gnu` installed — no `wasm32-unknown-unknown`
target, and no `wasm-pack`/`wasm-bindgen` — so the build command cannot be verified here and is not invented in
this brief. Missing inputs: the source at that revision, an isolated wasm target/toolchain, and a build recipe
(what the package publishes: a wasm-bindgen module plus its `.wasm`). The attempt needs a fetch/build authorization;
it would also have to reproduce the wrapper's JS API (`CogStream`, `tiles_for_window`, `decode_tile_f64`, `nodata`)
or the harness cannot load it.

**Deliberately adopt the published `0.5.1`.** This needs user approval to re-pin `candidates.json` and the plan's
naming of the pinned revision, and to record explicitly that the qualified artifact is a published build with no
source correspondence — the correspondence finding stays in the file rather than being deleted. Reusable without
re-measurement: the bounded-transport and analytic comparison *shape* (five windows, cell-exact expectation, nodata
validity, ledger reconciliation, refusal controls) and the host/asset identity of this one Linux WebKit build.
Not reusable as qualification: the pin-correspondence claim, and any statement about the pinned route or other
platforms. Because the artefact changes, its numeric/validity behaviour must be re-measured before it is called
qualified.

**Another engine or native route.** A separate user decision, not a fallback. The inspected alternatives are
`geolibre-wasm` (deferred by the plan, registry metadata only, not installed) and native GDAL (already allowed for
preparation/slope, which does not cover the bounded numeric/display role in the WebView). Neither was investigated
further here, and neither should be started without its own assignment.

### 4. Decision-ready recommendation

**Support the starting recommendation, with one qualification.** Retaining the declared pin is the only choice that
keeps the qualification claim meaningful: the pin is the reviewed source, and adopting a build whose provenance is
`gitHead` metadata alone would convert an unresolved correspondence failure into a silent substitution. The
qualification: the pin is currently *unbuildable from local material*, so "resolve its build feasibility" is a real
fetch-and-build action with its own risk, not a formality. If the user is unwilling to authorize that, the honest
alternative is the second choice — adopt `0.5.1` explicitly, keep the mismatch recorded, and stop calling the
result pin-qualified.

* **Approval needed next:** one isolated pinned-source build attempt (fetch source at `9c0ff4f…`, add an isolated
  `wasm32-unknown-unknown` toolchain, build, and record the resulting hashes) — or, alternatively, approval to
  re-pin to the published `0.5.1` with the correspondence failure preserved in `candidates.json`.
* **Most important remaining product risk:** that the artifact's numeric/validity behaviour on *managed* rasters —
  not the synthetic plane — and its working-memory behaviour under the plan's 1 GiB budget are unknown for whichever
  artifact is chosen. No browser or Desktop run has ever decoded a managed original.
* **One experiment that would reduce it after approval:** on the chosen artifact, one bounded numeric/validity and
  peak-memory measurement over a prepared derivative of a managed fixture, reusing the existing window/ledger
  comparison shape and the existing native preparation. That single measurement decides whether the route continues
  or the engine decision has to be revisited.
* **Documented unknown, accepted:** the pinned source's buildability, and whether published `0.5.1`'s API surface
  would even be reproduced by a build from `9c0ff4f…`. Neither is resolvable from local records, and this brief does
  not claim otherwise.
