...
S0 prototypes **Import → find → preview → add → inspect → remove → reuse in another Design** with real shared controls before backend expansion. Drive these transitions in the repaired gallery; screenshots alone are insufficient. Do not build another standalone HTML mockup or treat gallery stubs as proof of the native chooser, stored files or renderer performance.
...
| --- | --- | --- |
| **S0 — Baseline and workflow reference** | Fix actual gallery registration drift; inventory commands against section 1; build the two-surface workflow reference with existing dock controls and representative state fixtures. Record baseline Canopi/upstream timing on the same input. Own ui-gallery and shell reference fixtures; no runtime library migration yet. | Desktop/Web galleries mount without fatal errors; the library journey in section 6 is driven with keyboard/narrow/theme states. Removed features are absent from the target reference. Actual native baseline failures and measurement conditions are recorded separately. |
