...
Status: proposed — complete application migration proposal, not implementation or acceptance of changed contracts.
...
These are explicit proposal assumptions, pending user acceptance; they do not amend accepted ADRs merely by being written here:

1. Use GeoLibre's standard overview/resampling and mosaic display behavior. At overview scales, the picture may differ from Canopi's current reduction of the exact composed native grid. Numeric inspection, source priority, analysis and saved results remain exact under their existing rules. Section 5 specifies this narrow amendment.
2. Preserve current edition capabilities. Desktop retains local Data/Analysis; Web retains its existing Design, canvas, Catalog, Location and basemap capabilities and preserves unavailable local dataset references. Adding browser raster import/analysis is a separate product increment, not an implied requirement of using WASM on Desktop.
3. Preserve the existing Horn Slope operation, units, validity and quality rules. Do not silently substitute GeoLibre's differently calculated slope. GeoLibre-native algorithms can later become explicitly named recipes without reinterpreting saved definitions.
4. Preserve file format v6 and supported admission behavior. New library cache metadata may migrate additively; this work does not introduce an implicit `.canopi` converter or remove an existing feature.

If the user changes one of these choices, revise its affected sections before implementation. In particular, exact legacy display reduction would require a different display preparation contract; full Web processing would require browser storage/admission design; changing slope requires recipe versioning and new scientific expectations. Those are material choices, not implementer discretion.
...
| Analysis and result management | Keep named saved definitions, eligibility, job ownership, automatic refresh, cancellation, Retry and publication. Preserve Horn Slope. Adopt upstream algorithms only as qualified, explicit recipes. | UI → action → IPC → native operation → published result → map → inspection; failure preserves previous results. |
...
flowchart TD
  ...
  Analysis -. explicitly approved new recipes .-> Tools[GeoLibre Rust tools]
...
| `geolibre-rust` native tools | Native compilation is established. Do not ship an unused all-tools executable just to claim adoption, or change Horn behavior to increase reuse. For the first approved recipe that needs this suite, use its pinned native CLI as a job-owned child process; use argv, job scratch paths, bounded inputs, kill/reap and validated outputs. No Python/WASI virtual-filesystem detour on Desktop. |
...
Under the proposed decision in section 1, native-cell topmost-valid composition remains authoritative for numeric reads and analysis. Display uses upstream per-source overviews, the saved source priority and transparent NoData. It uses one layer-wide stretch, units and legend, not a different automatic scale per member. At low zoom this is a visualization of the collection, not its exact reduced numeric grid.
...
On approval/implementation, update only their display-reduction clause and link this decision; retain exact priority/validity, history and analysis rules. Mark old exact-display tests superseded by the accepted behavior, preserving their numeric cases. Do not quietly delete the tests to get the package green.
...
### Analysis, inspection and history

Keep existing exact resolver, projected-metre eligibility, full-resolution numeric input, Horn halo, degree/percent results and separate validity/quality masks. R50 Retry submits the stored definition/expected head, never current form fields. Jobs survive panel changes; cancellation is explicit; failed refresh retains the last complete result. Analysis publication rechecks source and definition identity inside the transaction.
...
Open Analyze from a selected eligible dataset or the Analysis rail command. The first route preselects the input; it does not immediately execute. With one supported recipe, show a clear Slope heading instead of a decorative one-option radio group. Present input, result name and degree/percent choice with eligibility reasons at the field that needs correction.
...
“LiDAR filters” in this delivery means understandable library/result filtering and presentation controls. It does not introduce LAS classification, ground extraction, numeric masking or new scientific recipes. Existing visibility/opacity remain saved in v6; new name/type/status filters are session view state. Do not add unspecified persistent color/range fields during this UI slice. If persistent value filters are requested, author their quantity, storage and analysis-independence contract before adding them.
...
4. Migrate representative existing library assets through the lazy derivative route. New imports prepare derivatives before commit. Preserve originals, IDs, history, saved analysis definitions and native numeric samples byte-for-byte where no new calculation is requested.
...
7. Additive library migrations must preserve rollback of authoritative data. The previous binary need not understand new disposable display-cache metadata; it must not be pointed at an incompatible authoritative schema without an explicit migration/backup decision. No destructive source cleanup is part of display-cache eviction.
...
| **S1 — Complete upstream display proof** | Qualify exact npm artifacts, MapLibre/Pixi compatibility, one real local derivative, Tauri range asset transport, worker ownership, offline initialization, CRS, layer stacking and style reload through production map callers. `maplibre/`, `app/canvas-map-surface/`, native asset descriptor command, build config. | Existing Canopi Design + editable plants + one real projected COG runs in the Desktop WebView. Pan/edit while rendering; dispose/reinstall during a read; reset style; no UI-thread heavy decode, native tile process churn, global GeoLibre store or unexpected network. Confirm Web bundle boundary and packaged-asset feasibility. Continue automatically if green. |
| **S2 — Library and display migration** | Derivatives, descriptors, ordered collection display, previous-composition compatibility, source-specific NoData, disk admission, leases, cache limits and recovery. `services/lidar/`, shared contracts/bindings and display adapter. | Existing and new library workflows work end to end, including multiple imports, history, large gaps, corrupted member, cancellation, capacity fault and retry. Exact numeric tests retain their values. Performance/resource gates pass on representative inputs. |
...
| **S4 — Whole-app composition and compatibility** | Exercise every section-3 capability while new geographic code is active. Repair migration regressions at their owners. Keep Horn and qualify exact command/IPC/native result boundary; do not add unused GeoLibre tools. | Saved v6 Design/native-Web roundtrips, canvas/plants/planning/Notebook/export journeys, old library compatibility, provider failures and lifecycle tests. No feature removed to accommodate upstream architecture. Delete superseded display code/switch after proof. |
...
### Native GeoLibre expansion after this same-feature delivery

The broader foundation is ready for more upstream reuse without exposing every tool. The first product-approved new terrain/vector/LiDAR operation uses the pinned Rust runner and its manifest for machine parameter facts, plus a small Canopi recipe for domain eligibility and publication. It gets an independently calculated fixture, method/version provenance, cancellation and resource proof before becoming available. This is the chosen future integration boundary; shipping additional analyses, point clouds, RGB imagery, hydrology, SQL, collaboration, mobile or multiple map engines is not required to preserve today's features.

If the user explicitly chooses upstream slope for new analyses, bring that first recipe into S4: retain legacy Horn definitions/Retry, add distinct saved method/version identity, use a two-cell halo for the inspected projected implementation, derive its actual quality policy, and qualify nonplanar/NoData/seam cases. A flat plane alone cannot distinguish those methods. Revise the persistence/recipe contract before coding rather than implementing this paragraph as an implicit change.
...
| --- | --- |
| Projected plane `z=x`, 1 m pixels | Native interior slope is 45° / 100%; inspection reports original values, including legitimate zero/negative elevations. Real command/publication test, not UI-only numbers. |
| Nonplanar terrain and a NoData hole | Existing Horn edge/quality behavior retained; no acceptance based solely on a plane shared by different algorithms. Numeric output compared with the independent Horn oracle and existing tolerances. |
...
Update affected operating guides in the same code delivery: MapLibre for manager/worker/asset ownership; LiDAR for derivatives, exact numeric boundaries and UX routing; edition development for the repaired gallery/real smoke; build-release for bundled assets/notices; document format only if the accepted storage contract actually changes. Reconcile ADR 0027's display clause only after its proposed amendment is accepted. Retain ADR 0025's shared camera, ADR 0024's PDF scope and current edition rules.
