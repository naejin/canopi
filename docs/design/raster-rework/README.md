...
The user selected useful GeoLibre components with **existing UI/features first**. G1–G5 is independently accepted at `a5fc7d7b`. Bounded generations delivered through `eb64a853` on `feature/bounded-raster-generations` have real caller changes but remain [partially reviewed, not accepted as complete](bounded-generation-review.md). Forward the [pagination/lifecycle completion assignment](bounded-generation-completion-agent-prompt.md) on the same `canopi-jv8a.4` bead/branch; do not restart the retired kickoff. The [design](bounded-generation-design.md) still requires retained standard source COGs, sparse resolved COGs and whole-workflow gates. The implementation receipt lives on that branch as `bounded-generation-receipt.md`; it is not duplicated into this planning checkout. Q remains frozen/unqualified. The [collaboration protocol](collaboration-protocol.md) preserves architecture/review, implementation and user courier authority.

| Artifact | Purpose |
| --- | --- |
| [Bounded-generation completion assignment](bounded-generation-completion-agent-prompt.md) | Sole current assignment: BG6-A and BG7-A/B/C on the existing feature branch, one consolidated completion |
