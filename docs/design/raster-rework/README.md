...
The user selected useful GeoLibre components with **existing UI/features first**. G1–G5 is independently accepted at `a5fc7d7b`. Bounded generations delivered through `1fcab504` on `feature/bounded-raster-generations` have real caller changes but remain [partially reviewed, not accepted as complete](bounded-generation-review.md). Forward the [BG1–BG5 correction assignment](bounded-generation-correction-agent-prompt.md) on the same `canopi-jv8a.4` bead/branch; do not restart the retired kickoff. The [design](bounded-generation-design.md) still requires retained standard source COGs, sparse resolved COGs and whole-workflow gates. The implementation receipt lives on that branch as `bounded-generation-receipt.md`; it is not duplicated into this planning checkout. Q remains frozen/unqualified. The [collaboration protocol](collaboration-protocol.md) preserves architecture/review, implementation and user courier authority.

| Artifact | Purpose |
| --- | --- |
| [Bounded-generation correction assignment](bounded-generation-correction-agent-prompt.md) | Sole current assignment: BG1–BG5 on the existing feature branch, one consolidated correction |
| [Bounded-generation independent review](bounded-generation-review.md) | `1fcab504` disposition, source evidence and limits of independently repeated tests |
| [Bounded-generation delivery record](bounded-generation-receipt.md) | Consolidated BG1–BG5 response: behavior, evidence, admission boundaries and unavailable gates |
| [Bounded-generation initial assignment](bounded-generation-agent-prompt.md) | Retired kickoff; continue the correction, not a new bead or baseline || [Bounded-generation design](bounded-generation-design.md) | Main-agent decisions: storage, compatibility, lifecycle, scientific/display semantics, resource policy and gates |
