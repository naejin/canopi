...
Status: implemented under bead `canopi-85wo`. Qualification follow-up `canopi-a5tj` now has exact-fixture browser evidence and partial Linux native evidence. Release qualification remains open because the browser performance references are missed, matched three-run baselines and high-DPR capacity runs are absent, physical input on the shared-GPU native path was blocked by the locked session, and macOS/Windows are unavailable.
...
Baseline: `0cb48bb3633508e888d03b1ffd480b0b0ca83490`, branch `feature/single-location-confirmation`, tracking the matching origin branch. User-owned changes present during planning: `.beads/issues.jsonl`, `docs/README.md`, and untracked `docs/design/v2-session-prompt.md`. Preserve them. The current checkout contains accepted location-confirmation and shared-workspace work; a future implementation branch must retain that work even if then-current main lacks it.
...
These runs do not satisfy the performance gate. At DPR 1, frame-opportunity/input-to-second-RAF p95 values were 116.6/123.7 ms for the representative shared renderer, 50.0/64.1 ms for representative Canvas2D, 66.7/180.0 ms for dense 10,000 Plants, and 1,000.1/2,005.3 ms for dispersed 10,000 Plants. Every scenario therefore missed the diagnostic proxy overall; representative shared and dispersed also recorded sustained stalls. DPR 1.5 and 2 proved DPR-sensitive correctness only: the headed background window was throttled to about 1 Hz, so those timings are not treated as rate evidence. High-DPR 10,000-Plant scenarios, three matched serial runs, and a before/after baseline remain outstanding. Aggregate-only receipts are kept outside Git under `/tmp/canopi-world-qualification-final-*.json`; fixture contents and captures were not committed.
...
Exit gate: all relevant local quality gates and available real-system checks pass, integration contains every required accepted fix, measured limits and platform gaps are recorded, and no resource/content regressions remain. Update `docs/agent/canvas-runtime.md`, `docs/agent/maplibre.md`, the canvas-workspace interface guide and performance guide when commands/scenarios change; replace stale constructor/provisional-shell statements. Link this plan from `docs/README.md` when its user-owned edit can be safely integrated. No operating-guide changes are made merely to publish this proposal.
