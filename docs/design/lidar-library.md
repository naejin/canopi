...
The analysis extension now targets local processing of user-owned drone TIFFs, with an [IGN-backed POC](lidar-agroecology/ign-poc.md) to validate the end-to-end path. See the [current delivery recommendation](lidar-agroecology/delivery-strategy.md). This does not change the accepted library behaviors below or imply that analysis is implemented.
