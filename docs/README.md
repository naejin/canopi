...
The [proposed raster, Data and Analysis rework](design/raster-data-analysis-rework.md) records simplified bounded import, selective GeoLibre native integration, Desktop workbenches, and Web Location scope. Native extraction/postprocessing is independently accepted at `a5fc7d7b`. The revised [bounded-generation assignment](design/raster-rework/bounded-generation-agent-prompt.md) and [design](design/raster-rework/bounded-generation-design.md) own the next end-to-end foundation batch: retained standard COGs and sparse resolved COG chunks replace the unimplemented custom encoding, with existing UI preserved and capacity enabled only after verification. [ADR 0026](adr/0026-sparse-raster-generations.md) records its storage trade-off. The old Q harness is frozen, not its prerequisite. The [raster rework handoff folder](design/raster-rework/README.md) retains user-requested review/debrief evidence separately from execution status in bd. New workbench and basemap UI remains later work.
