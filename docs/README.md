...
The [raster, Data and Analysis rework](design/raster-data-analysis-rework.md) retains broader future workbench/Web scope. The user-selected [ordered COG design](design/raster-rework/ordered-cog-design.md) is implemented on `feature/bounded-raster-generations` and replaces materialized source merging; the [delivery receipt](design/raster-rework/ordered-cog-receipt.md) owns its evidence and limits, and [ADR 0027](adr/0027-ordered-cog-data-layers.md) records the change. Historical dense/sparse generations remain readable. Native integration is accepted at `a5fc7d7b`; independent disposition and the integration decision remain pending, so the branch is not integrated into `main`. Q stays frozen. The [handoff folder](design/raster-rework/README.md) and [debrief](design/raster-rework/review-and-debrief.md) retain revision-linked evidence and the improvement record; bd owns execution state.
