...
The proposed [whole-application GeoLibre adoption plan](design/raster-rework/geolibre-adoption-plan.md) addresses the subsequent display-performance and UX reports while preserving all current Canopi capabilities. It maps upstream reuse, domain ownership, migration, Data/Analysis/Layers UX, TDD, whole-app acceptance and delivery. Its explicit display-semantic amendment needs acceptance before implementation; it does not yet override current guides, edition boundaries or scientific contracts. The [debrief](design/raster-rework/review-and-debrief.md#whole-rework-delivery-and-improvement) retains measured outcomes; bd owns execution state.
