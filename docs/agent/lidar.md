...
The current repair baseline is `783e31e3` on `feature/bounded-raster-generations`, retaining accepted native integration `a5fc7d7b` and C1/C2. The [independent review](../design/raster-rework/ordered-cog-review.md) requires eleven repair groups despite reported green gates. The design's repair contract settles completion, migration Undo limits and dependent recomputation; `canopi-kko3` needs caller evidence before acceptance. This planning checkout does not contain the delivery implementation. Preserve the branch's newer source/owner inventories, compatibility-lease guidance and revision-labelled receipts when merging this documentation; the inventory below describes the earlier checkout.
