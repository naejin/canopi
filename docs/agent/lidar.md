...
The [completion prompt](../design/raster-rework/completion-agent-prompt.md) is the sole current assignment under `canopi-j571`: integrate the accepted stack, qualify larger capacity, implement production workbenches/inspection and Web Location/providers, then verify one combined candidate. Its [contract](../design/raster-rework/completion-design.md) owns conditional capacity, execution and review boundaries. These future changes are not present merely because the handoff exists. Preserve R12–R14's tile footprint, edit lifetime, traversal and teardown regressions and the mounted-map evidence in the [receipt](../design/raster-rework/ordered-cog-receipt.md).
