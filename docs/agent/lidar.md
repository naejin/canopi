...
The user selected [compatibility-first GeoLibre native integration](../design/raster-rework/geolibre-integration-design.md). The [current implementation prompt](../design/raster-rework/geolibre-integration-agent-prompt.md) under `canopi-jv8a.1` owns G1–G5: use the GeoLibre-selected `wbgeotiff` native core for import extraction and slope postprocessing while preserving existing UI, IPC, persisted bytes and capacity limits. This is accepted direction, **not a claim that the code has migrated**. GDAL remains preparation, CRS, slope and display authority. The implementer updates this guide after the production callers actually change.
