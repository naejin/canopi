...
The user selected [compatibility-first GeoLibre native integration](../design/raster-rework/geolibre-integration-design.md). G1–G5 was delivered at `3004e4d3` on `feature/geolibre-native-raster-integration`, not integrated into this checkout. The [current follow-up](../design/raster-rework/geolibre-integration-followup-agent-prompt.md) addresses the [independent review](../design/raster-rework/geolibre-integration-review.md): combined temporary-disk accounting and accurate cleanup claims. Preserve the delivered guide's production descriptions when applying this handoff on that branch. GDAL remains preparation, CRS, slope and display authority; UI, IPC, persisted layouts and limits stay unchanged.
...
Rust owns native raster work; TypeScript owns any newly required orchestration. No new Python raster tooling or wholesale Python removal. Existing unrelated repository tools remain in use. Native dependency source/build and the design's isolated small integration tests are authorized by the forwarded assignment; large-capacity, new UI and other engine work are not.
