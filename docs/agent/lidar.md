...
Engine, artifact and capacity qualification for the rework lives in the [Q receipt](../design/raster-qualification-q.md), its harness under `scripts/raster-qualification/`, and the [GeoLibre reuse inventory](../design/raster-rework/geolibre-reuse-inventory.md). That receipt is the input N1 depends on; read it before planning bounded numeric access. Its load-bearing findings: bounded numeric and display access is qualified for **prepared tiled COG derivatives** only, because no candidate role range-reads a stripped GeoTIFF, so N1 must either prepare before a head becomes numeric-readable or supply a bounded legacy reader; GDAL is the CRS resolver, and the incomplete EPSG:2154 fixture WKT is neither rewritten nor resolvable by the candidate projection engine; no published `whitebox-wasm` artifact matches the pinned commit; the candidate stream decoder's `free()` is not idempotent, so the owning adapter must make teardown safe; and the Whitebox WASI `slope` tool is rejected for edge interpolation.

The harness is a qualification tool, not a production path. Its verdicts are fail-closed and its regression tests are part of the gate: `python3 -m unittest discover -s scripts/raster-qualification/tests` must pass before any verdict from it is trusted, and `bash scripts/raster-qualification/run_all_experiments.sh` reruns every experiment.
