...
The current repair baseline is `524eef55` on `feature/bounded-raster-generations`, retaining native integration `a5fc7d7b`, C1/C2 and the eleven-group repair at `64050896`. The [current review](../design/raster-rework/ordered-cog-review.md#current-disposition-at-524eef55) leaves tile-footprint filtering, async panel ownership and mounted-map evidence. Dependent recomputation has positive independent caller and saved-profile evidence; broad acceptance remains pending. Preserve the delivery branch's newer inventories and receipt when incorporating this documentation. This guide's owner table below belongs to the earlier planning checkout and must not replace that branch's implementation inventory; on that branch reconcile current symbols such as `apply_member_edit` and remove statements claiming its implementation is absent.
