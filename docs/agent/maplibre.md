...
- Web Edition v1 omits visible Location editing entirely. Do not mount a Web Location map, coordinate form, or map-picking workflow, and do not import `app/location/coordinate-workbench.ts` from Web sources. Saved `.canopi` Location data round-trips, but `WebCanvasWorkspace` currently mounts no canvas basemap. Street maps are permitted scope, not an implemented Web Canvas capability; see [ADR 0013](../adr/0013-web-edition-map-scope.md). See `docs/adr/0016-web-edition-omits-geocoding.md`.
