...
- Web Edition Location UI mounts through `web/WebLocationWorkspace.tsx`, uses the browser-safe `app/location/coordinate-workbench.ts` seam, and requests `useLocationMapEditingHost(..., { basemapStyle: 'street' })`. Do not import the desktop `app/location` index, `search-controller`, `LocationTab`, or `LocationInput` from web sources.
