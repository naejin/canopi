...
- Map surfaces and readiness/status UI consume the confirmed Location projection of the Design spatial frame through the Location Workbench; do not import Design Session state directly for location presentation.
...
- A new Design persists the canonical provisional anchor. The current Location Workbench projects only confirmed placement into the Canvas Map Surface, so a new Design cannot create MapLibre or issue remote source requests before explicit placement confirmation.
