...
- Planned Canvas PDF map output follows [ADR 0024](../adr/0024-shared-canvas-pdf-export.md): an optional street-basemap print Layer for Designs with a saved Location on desktop and Web, with satellite imagery, contours, and hillshade deferred. Preserve required attribution and validate the map source for the requested print coverage and resolution; interactive tile loading alone does not establish PDF suitability.
