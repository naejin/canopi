...
- Publishing and release workflows obtain schema metadata and the immutable prepared DB asset name through `scripts/species_catalog_contract.py value` and verify databases through `verify-db`; do not parse generated Rust or duplicate inline checks. `prepare-db.py` verifies exact source-export bytes through the same contract compiler before opening the export, and operators can run `verify-source-export` independently. The DB publisher must stage the exact derived asset basenames and must not overwrite an existing identity asset.
