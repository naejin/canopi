...
- `prepared_raster.rs` also owns the free-space guard: it reserves the padded derivative size, a 4 MiB metadata prefix ceiling and a 256 MiB floor, and rechecks between bounded writes. `paths.rs` reports capacity with `statvfs` (unix, `libc`) or `GetDiskFreeSpaceExW` (windows, `windows-sys`); a platform that cannot report it fails preparation by name. Do not replace this with a guess.
