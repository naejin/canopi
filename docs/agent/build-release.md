...
- `cd desktop/web && npm run check:types` compiles the same expected set in read-only mode and reports every stale destination. It does not refresh files and does not rely on a subsequent `git diff` to detect drift.
...
- During publication, `target/bindings-gen-publication.in-progress` records the operation. A later generate or check refuses to continue while this marker exists. Inspect the generated-file diff, restore or accept the interrupted result, remove any operation-owned `.bindings-gen-*` sidecars, then remove the marker before retrying.
