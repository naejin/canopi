...
# Web Species Catalog artifact-contract/generated-admission drift
...
- The Web Edition Species Catalog command is `cd desktop/web && npm run generate:web-catalog`. It emits ignored DuckDB-queryable Parquet catalog shards under `desktop/web/public/canopi-catalog/` from the authored `common-types/web-species-catalog-artifact.json` contract; run it from a checkout with local canopi-data exports when catalog assets are needed for packaging or adapter testing. Refresh all checked-in contract adapters, including the browser/Node admission module, with `npm run gen:types`; use `python3 scripts/web_catalog_artifact_contract.py check` for a focused authored-contract/admission drift check.

## Generated Contract Publication

- `cd desktop/web && npm run gen:types` is the write interface for the complete generated contract set. The compiler renders every Rust and TypeScript artifact, validates the Python-owned contracts, formats generated Rust, and stages the Python-owned Web Catalog files before changing any checked-in destination.
- `cd desktop/web && npm run check:types` compiles the same expected set in destination-read-only mode and reports every stale destination. It does not refresh files and does not rely on a subsequent `git diff` to detect drift. Generate and check coordinate through the persistent ignored `target/bindings-gen-publication.lock`: publication takes exclusive admission, checks take shared admission, and a conflicting operation fails before any destination snapshot or read.
- Publication replaces the staged files one destination at a time and restores the complete prior set when an error is reported. This is rollback-safe, but it cannot make one atomic filesystem transaction across the frontend and backend directories. A process or machine crash can therefore expose a partial set.
- Before destination snapshots or staging, publication flushes `target/bindings-gen-publication.in-progress`; it also syncs the marker's parent directory when the host platform and filesystem expose portable directory syncing. Platforms or filesystems without that support retain best-effort machine-crash evidence, so always inspect the generated-file diff after an abnormal stop. A later generate or check refuses to continue while the marker exists. Inspect the diff, restore or accept the interrupted result, remove any operation-owned `.bindings-gen-*` sidecars, then remove the marker before retrying. Keep the persistent `.lock` file; it contains no publication state and must not be manually deleted between operations.
