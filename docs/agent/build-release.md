...
- Admission caps a source at 512 MiB, a selection at 16 files/1 GiB, and a union envelope at 25 million cells; it is one policy in `desktop/src/services/lidar/admission.rs`, enforced during selection validation, the managed-original copy and verification, before review work that depends on the union, and rechecked at Apply. These remain safety limits for new input, not drone-scale performance claims. A new source generation publishes an ordered composition of retained source COGs and materializes no merged raster, so composition, reorder, remove, undo and restore are not area-bounded; reads, display, slope and the preserved dense adapters remain bounded per window.
...
- The real-fixture lifecycle test runs with: `cargo test -p canopi-desktop services::lidar::e2e::e2e_import_publish_slope_restart_reuse -- --ignored --exact --nocapture` (requires the GDAL tools and the IGN MNT fixture under `~/Downloads`). It checks decoded preview/tile alpha, restart files, same-file replacement, targeted undo, deletion, and a 45° analytical plane. Set `CANOPI_LIDAR_E2E_FIXTURE` to run it against an explicit tile instead of the documented default; report the tile and hash actually used.
