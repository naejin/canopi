...
# Strictly verify the locally prepared or downloaded bundled DB
...
The [production PDF workflow](../../.github/workflows/pdf-production-probe.yml) runs on `main` pushes and pull requests affecting PDF code, canvas input, frontend dependencies, font preparation, the native WebView harness, or the validation host. Keep push and pull-request path filters aligned when those inputs move. It can also be dispatched manually. The superseded encoder-comparison workflow and fixture were retired after the production gate replaced them.
...
- Promotion admits only a successful completed candidate workflow, matching repository/version metadata and an exact source commit. New drafts target that commit; existing tags must resolve to it. Published releases cannot be replaced. Run `python3 -m unittest scripts.test_promote_release` after changing promotion; Build & Test runs this fake-GitHub CLI coverage too. Packaged smoke evidence remains a separate requirement in `docs/release.md`.
- `promote-release.sh --artifact-dir <downloaded-candidate-root>` reuses local package bytes while still fetching the authoritative manifest from GitHub. `scripts/release_candidate_artifacts.py` owns manifest package admission for both downloaded and local paths: it rejects unsafe paths and duplicate asset names, verifies every listed checksum, and admits only listed installers as sources for release preparation. Local packages are copied into operation-owned staging while hashing; never upload directly from mutable caller-owned files after verification. A missing or mismatched local package fails before release mutation, with no silent download fallback. Relative artifact roots use the caller's working directory, not the repository root. Keep the fake-GitHub CLI tests covering both paths and source mutation between verification and upload.
- Promotion requires the complete desktop target/format set in `STABLE_PACKAGES` in `scripts/release_candidate_artifacts.py`. It snapshots verified installers for both local and downloaded inputs, preserves original filenames, and creates six byte-identical stable copies for website links. Unknown/missing formats and stable-name collisions fail before release mutation. Keep this mapping, the candidate matrix, promotion tests, and the filename table in `docs/release.md` coherent when targets change.
- `SHA256SUMS.txt` remains the authoritative candidate manifest; generated `RELEASE-SHA256SUMS.txt` uses public basenames and covers originals plus stable copies. Public stable app releases must be published with `gh release edit v<version> --draft=false --latest`. DB-only releases and prereleases must not become latest. Website buttons use `/releases/latest/download/<stable-name>` after the first compatible release is live; promotion does not change the website or existing published releases.
...
- Keep each worktree's Cargo target directory separate. `bindings-gen` embeds its source checkout through `CARGO_MANIFEST_DIR`; sharing a target directory can reuse an executable that reads a different or deleted checkout. If that happened, run `cargo clean -p bindings-gen` in the intended checkout before `npm run check:types` or `npm run gen:types`.
...
For command-to-service tests, use Tauri's test support to build managed state: `tauri::test::mock_builder().manage(service).manage(executor).build(tauri::test::mock_context(tauri::test::noop_assets()))`. Import `tauri::Manager` and invoke the actual command with `app.state()` arguments through `tauri::async_runtime::block_on`. Enable Tauri's `test` feature only for development/test dependencies when needed. Keep the real executor and service; release catalogue locks before waiting for workers. Calling `executor.run` with a copied command body does not prove command wiring. Direct command invocation proves neither IPC serialization nor handler registration; use the authored frontend wrapper/transport tests and native command policy for their respective boundaries.

For publication-preservation claims, create a valid small publication through the real worker, wait for terminal settlement and release of cancellation guards, and verify it through the real reader before injecting the fault. After refusal or failure, verify readable old output and absence of a partial replacement. Placeholder paths or manifest rows can test catalogue queries, but cannot establish that a usable publication survived.
...
- Review `desktop/tauri.conf.json` CSP when adding resource types or origins. Scripts are restricted to the app plus `'wasm-unsafe-eval'` for the raster engine; image and connection directives currently allow HTTPS sources. Keep native asset access scoped to the image cache.
- Asset protocol scope is limited to the app-data image cache and published LiDAR display COGs (`$APPDATA/lidar/display-cog/*.tif`).
...
- The LiDAR library shells out to the GDAL command-line tools (`gdalinfo`, `gdal_translate`, `gdaldem`, `gdaltransform`) behind the narrow adapter in `desktop/src/services/lidar/engine.rs`. Fixed argv, no shell, bounded stdin/captured-output/temp-output growth and duration, cancellable while running; engine discovery is cached and its version is recorded in manifests.
- `desktop/THIRD_PARTY_NOTICES.md` is bundled as a resource and names the adopted raster packages and the GeoLibre CLI with their exact versions and licenses; `third-party-notices.test.ts` fails when an installed version or the recorded GeoLibre revision drifts from it. Update it with any raster dependency change.
- New slope results (recipe 2) run the pinned GeoLibre CLI (`geolibre-rust` `aac2b743`, `geolibre-cli` 1.5.3) through `desktop/src/services/lidar/geolibre.rs`. Build it with `scripts/build-geolibre-cli.sh`, which writes `desktop/binaries/geolibre-<host-triple>` (git-ignored; the name Tauri uses for a bundled sidecar) and prints the `CANOPI_GEOLIBRE_BIN` export for development. Discovery order is `CANOPI_GEOLIBRE_BIN`, beside the application executable, then `PATH`. Release bundling of the sidecar is not wired yet (see bd); without the CLI, new slope is unavailable by name while saved results stay readable. The build script's revision must match `GEOLIBRE_REVISION` (unit-tested).
- GeoLibre lanes are ignored by default and need both engines: `CANOPI_GEOLIBRE_BIN=<geolibre> cargo test -p canopi-desktop -- --ignored geolibre` runs the analytic-surface/seam/quality/provenance test and the mid-run cancel/reap test.
- Import extraction and slope result postprocessing additionally read through the pinned native tiled core `wbgeotiff` (git dependency on `opengeos/whitebox-wasm`, revision recorded in `desktop/Cargo.toml` and `Cargo.lock`), reached only through the private reader in `desktop/src/services/lidar/prepared_raster.rs`. It prepares one uncompressed single-band Float32 COG (256×256 blocks, no overviews, `GDAL_PAM_ENABLED NO`) and streams it in at most 1024×1024 windows. The derivative is temporary; no whole-raster buffer is created on either path.
- `prepared_raster.rs` also owns the free-space guard: one checked estimate covers the padded derivative size, a 4 MiB metadata prefix ceiling, any numeric output written while the derivative is alive, and a 256 MiB floor, enforced before preparation and rechecked at every window boundary. A measured check is not an OS reservation; concurrent disk consumption still surfaces as an ordinary write error.
- A slope job's `prepared/analysis/<definition_id>/staging-*` root is owned by a guard that removes it on any early return, propagated error, cancellation or panic, and is disarmed only once the directory is renamed into its published generation directory; startup pruning removes roots left by a crash (`canopi-jv8a.3`).
- `paths.rs` reports capacity with `statvfs` (unix, `libc`) or `GetDiskFreeSpaceExW` (windows, `windows-sys`); a platform that cannot report it fails preparation by name. Do not replace this with a guess.
- Development OSes need GDAL CLI tools on `PATH` (or `CANOPI_LIDAR_GDAL_BIN` pointing at a directory containing them). Slice 1 validated GDAL 3.8.4 on Linux; packaged-OS bundling of the engine is tracked as a follow-up bead and is required before release. The COG creation options used by preparation require a GDAL whose COG driver accepts `BLOCKSIZE`, `COMPRESS`, `OVERVIEWS=NONE`, `STATISTICS` and `SPARSE_OK` (3.7+; validated on 3.8.4).
...
- Admission caps a source file at 2 GiB, a selection at 24 files and 2 GiB together, and the proposed active collection at 400,000,000 processing cells charged per occurrence; it is one policy in `desktop/src/services/lidar/admission.rs`, enforced when the selection is admitted, again while each source is prepared, and rechecked at publication. The 25,000,000-cell union envelope guard survives only for the preserved dense format, which production no longer publishes. These remain safety limits for new input, not drone-scale performance claims. A new source generation publishes an ordered composition of retained source COGs and materializes no merged raster, so composition, reorder, remove, undo and restore are not area-bounded; reads, display, slope and the preserved dense adapters remain bounded per window.
...
- A generation becomes authoritative only after its member rows, promoted source references and the catalogue head publish in one transaction. Failure publishes nothing and keeps the prior head.
- The real-fixture lifecycle tests are ignored by default and need their fixtures named explicitly. The whole GDAL-backed lane runs with `CANOPI_LIDAR_E2E_FIXTURE=<IGN MNT GeoTIFF>`, `CANOPI_LIDAR_MNH_DIR=<directory of IGN MNH tiles>` and `CANOPI_LIDAR_CAPACITY_PLANE=<synthetic 400M-cell plane>` set, plus `CANOPI_LIDAR_GDAL_BIN` when GDAL is not on `PATH`: `cargo test -p canopi-desktop --lib -- --ignored --test-threads=1 --nocapture`. Individual lanes are `services::lidar::e2e::e2e_import_publish_slope_restart_reuse` (dense lifecycle, then the shipped ordered route over that grandfathered dense head), `services::lidar::e2e::e2e_sparse_generation_lifecycle`, `services::lidar::e2e::e2e_mnh_batch_import_apply_display_restart` (twelve real tiles) and `services::lidar::e2e::e2e_capacity_plane_import_display_and_bounded_reads`. They check that display derivatives prepare with valid values, restart reuse, same-file replacement, ordered membership and Undo, a 45° analytical plane, and the plane's block seams and declared NoData holes. Report the tile and hash actually used, and never present a fixture-less run as a pass.
- Tests that exercise the GDAL-backed bounded reader and its callers are ignored by default: `cargo test -p canopi-desktop services::lidar::prepared_raster -- --ignored`, `services::lidar::import::tests -- --ignored`, and `services::lidar::analysis::tests -- --ignored`. The hermetic part of `prepared_raster` (window traversal, malformed layouts, cancellation, cleanup, capacity) runs in the default suite.
