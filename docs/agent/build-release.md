...
- The Web Edition Species Catalog command is `cd desktop/web && npm run generate:web-catalog`. It emits ignored DuckDB-queryable Parquet catalog shards under `desktop/web/public/canopi-catalog/` from the authored `common-types/web-species-catalog-artifact.json` contract; run it from a checkout with local canopi-data exports when catalog assets are needed for packaging or adapter testing. Refresh the checked-in browser/Node admission module with `python3 scripts/web_catalog_artifact_contract.py emit --write`; `npm run gen:types` and `npm run check:types` include that output.
