...
- Admission caps a source file at 2 GiB, a selection at 24 files and 2 GiB together, and the proposed active collection at 400,000,000 processing cells charged per occurrence; it is one policy in `desktop/src/services/lidar/admission.rs`, enforced when the selection is admitted, again while each source is prepared, and rechecked at publication. These remain safety limits for new input, not drone-scale performance claims. An item publishes one ordered composition of retained source COGs and materializes no merged raster; reads, display and slope remain bounded per window.
...
- The real-fixture lifecycle tests are ignored by default and need their fixtures named explicitly. The whole GDAL-backed lane runs with `CANOPI_LIDAR_E2E_FIXTURE=<IGN MNT GeoTIFF>` and `CANOPI_LIDAR_MNH_DIR=<directory of IGN MNH tiles>` set, plus `CANOPI_LIDAR_GDAL_BIN` when GDAL is not on `PATH`: `cargo test -p canopi-desktop --lib -- --ignored --test-threads=1 --nocapture`. Individual lanes are `services::lidar::e2e::e2e_import_publish_slope_restart_reuse` (import, GeoLibre slope, restart, a second item from the same file, delete), `services::lidar::e2e::e2e_sparse_generation_lifecycle` and `services::lidar::e2e::e2e_mnh_batch_import_apply_display_restart` (twelve real tiles). They check that display derivatives prepare with valid values, restart reuse, fixed items and ordered membership. Slope lanes also need `CANOPI_GEOLIBRE_BIN` (or the CLI beside the test binary). Report the tile and hash actually used, and never present a fixture-less run as a pass.
- Tests that exercise the GDAL-backed bounded reader and its callers are ignored by default: `cargo test -p canopi-desktop services::lidar::prepared_raster -- --ignored`, `services::lidar::import::tests -- --ignored`, and `services::lidar::analysis::tests -- --ignored`. The hermetic part of `prepared_raster` (window traversal, malformed layouts, cancellation, cleanup, capacity) runs in the default suite. CI job `lidar-native` (`.github/workflows/build.yml`) installs GDAL 3.8.4 (Ubuntu 24.04 `gdal-bin`), builds the pinned GeoLibre CLI and runs `cargo test -p canopi-desktop --lib services::lidar -- --ignored --test-threads=1 --skip e2e_`; the fixture e2e lanes stay local.
