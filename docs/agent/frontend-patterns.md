...
- Canvas and panel rails share `--chrome-rail-width`, inset rounded controls, 20px icons, and mirrored ochre active indicators. The bottom launcher uses the same panel surface and control states; retain its text labels and compact horizontal layout. Panel headers share the panel surface and spacing scale. Catalog filter labels reserve a readable column and wrap at word boundaries before breaking long words. Empty plant-photo placeholders stay compact; loaded photos retain their full aspect ratio.
...
- Plant Symbol selection uses a separate toolbar popover after Plant Color. Show twelve botanical and four abstract choices in two labeled groups within one four-column keyboard listbox. Every choice, including Neutral dot, uses selected state; only the active option is a tab stop. The body scrolls within the viewport-clamped popover while its header and explicit apply actions remain visible. Arrow keys preview choices, while the two explicit apply actions distinguish selection from species default. Labels come from the shared symbol IDs and all eleven locales.
- Plant Symbol picker options render in neutral UI ink with ochre selection feedback and a separate preview showing the chosen symbol in the effective plant color. Plant Color previews use the effective selected symbol. Pass the actual glyph size to `PlantSymbolGlyph` so compact legend marks omit fine detail; native SVG attributes must preserve opaque fill and nonzero winding. Both popovers expose arrow-key choice navigation, retain explicit apply actions, and use `useAppearancePopover` to clamp their position and expanded height to the viewport; its component lifetime owns resize/scroll cleanup and moves focus into the dialog once on opening, without stealing focus on resize. Advanced color drags own pointer identity and release listeners on pointerup, pointercancel, window blur, selection changes, advanced-section closure, dialog closure and unmount. The toolbar button may show the current shared symbol when selected editable plants agree; mixed selections use a generic symbol affordance and explain the mixed/inherited state in the popover.
...
- Layer visibility and locks are distinct from the active editing layer. Keep hidden-layer controls readable, preserve active-layer emphasis when locked, expose visibility/active state accessibly, and show opacity as a percentage beside its labeled slider.
