...
- Budget, Timeline, and Consortium UI should consume the Planning Projection surface hooks (`useBudgetPlanningSurface()`, `useTimelinePlanningSurface()` through the Timeline Action Workbench, and `useConsortiumPlanningSurface()`) instead of importing `currentDesign`, canvas query surfaces, runtime mirror revisions, or planning Target presentation helpers directly.
- The Planning Projection module owns derived planning rows, Timeline Action grouping/layout read-models, Timeline species picker options, runtime input assembly, and planning Target presentation for timeline, budget, and consortium views.
