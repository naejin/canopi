...
- The implemented browser shell command set is New, Open `.canopi`, Download `.canopi`, language/theme, and panel toggles. [ADR 0024](../adr/0024-shared-canvas-pdf-export.md) approves adding Canvas PDF export through a browser download capability; its renderer and command are not implemented yet. Browser Drafts remain internal autosave/recovery state; do not expose a visible Drafts command or list. Keep reveal-in-file-manager, updater, Problem Report, native save/save-as, direct save-back, Design Location, and native file-management export flows outside the web shell. Other Web export exclusions follow ADR 0024.
