...
- Plant Symbol picker options render in neutral UI ink with ochre selection feedback and a separate preview showing the chosen symbol in the effective plant color. Plant Color previews use the effective selected symbol. Both popovers expose arrow-key choice navigation, retain explicit apply actions, and use `useAppearancePopover` to clamp their position and expanded height to the viewport; its component lifetime owns resize/scroll cleanup. The toolbar button may show the current shared symbol when selected editable plants agree; mixed selections use a generic symbol affordance and explain the mixed/inherited state in the popover.
