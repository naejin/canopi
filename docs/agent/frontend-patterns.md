...
- Timeline Action canvas input ordering, selected Timeline Action identity, stale-selection cleanup, Target Presentation writes, popover open/save/delete orchestration, keyboard delete handling, and cleanup belong in `app/timeline/interaction-frame.ts`. `app/timeline/canvas-workbench.ts` owns planning input assembly, layout offsets, local Preact state storage for render/tooltip/popover projection, and the small frame adapter seam. `InteractiveTimeline` should stay a canvas host plus render call.
