...
- Web Edition v1 Plant Detail may show one lazy-loaded remote hero image per Species when metadata is available. Show source/credit access with the image, tolerate missing or failed images cleanly, and do not assume the desktop native image cache exists in the web build.
