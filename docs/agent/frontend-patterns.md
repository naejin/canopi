...
- Browser-local Web Edition app data lives behind `desktop/web/src/web/browser-app-data.ts`. `WebApp` owns loading/persisting browser settings and installing the browser Design Session autosave observer; shell components receive command handlers instead of importing `localStorage`, file adapters, or app-data stores directly.
...
- Web Edition canvas mounting lives under `desktop/web/src/web/`. Use `WebCanvasWorkspace`, `WebCanvasToolbar`, and `browser-canvas-runtime.ts` instead of desktop `CanvasPanel`, `CanvasToolbar`, `BottomPanel`, `DisplayLegend`, `CommandPalette`, command registry, desktop LayerPanel, or document-session lifecycle modules.
- Web Edition Species UI mounts through `WebSpeciesCatalogPanel`, not the desktop `PlantDbPanel`, `FavoritesPanel`, or full `PlantDetailCard`. Its catalog controls should move toward the desktop Species Catalog Workbench filter shape by reusing Workbench-backed search/filter components or equivalent Workbench projections where the current Web catalog data supports them: text search, climate zone, habit or growth form, life cycle, active chips, and More Filters affordances only for backed fields. Hide unsupported desktop-only filters entirely in Web Edition instead of rendering disabled controls. Keep browser-local favorites/recently viewed Species Web-specific, and keep Web Edition Plant Detail reduced rather than importing the desktop `PlantDetailCard`.
- On mobile and small-screen Web Edition layouts, the Species Catalog Workbench may render as a bottom sheet. Keep the Species list immediately usable in that constrained height: collapse the supported filter controls into a one-line summary by default, expose active-filter state in that summary, and let users expand the full filter controls when needed. Do not require native HTML drag/drop for touch placement; provide a touch-friendly Place action that starts Plant Stamp from the selected Species, then lets the user tap the canvas to place it. Pointer devices may keep drag/drop as an additional placement path.
...
- Web Edition v1 omits visible Location editing entirely. Only desktop Location input/search surfaces should consume the search-capable `app/location/workbench.ts` or coordinate workbench. See `docs/adr/0016-web-edition-omits-geocoding.md`.
