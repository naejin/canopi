...
- The v1 browser shell command set is New, Open `.canopi`, Download `.canopi`, Drafts, language/theme, and panel toggles. Do not expose reveal-in-file-manager, updater, Problem Report, native save/save-as, direct save-back, or desktop-style export commands in the web shell.
- The Web Edition Browser App Shell should preserve desktop chrome muscle memory: desktop-style top bar structure, menu/dropdown interaction, centered Design identity, and the right-side PanelBar pattern. Match the desktop UX shape while backing it with web-safe command projections.
- Keep the shell thin: it should route through the App Command Graph, Design Session actions, browser file/storage adapters, settings seams, and panel chrome projections rather than duplicating command availability checks or importing Tauri IPC, IndexedDB, localStorage, or browser file APIs in generic components.
- The Browser App Shell is app chrome, not website navigation and not a reduced catalog widget. It should preserve the real Canopi workspace while making browser-local Drafts and explicit `.canopi` download/export clear.
- Browser-local Web Edition app data lives behind `desktop/web/src/web/browser-app-data.ts`. `WebApp` owns loading/persisting browser settings and installing the browser Design Session autosave observer; shell components receive Draft summaries and command handlers instead of importing `localStorage`, file adapters, or app-data stores directly.
- Web Edition canvas mounting lives under `desktop/web/src/web/`. Use `WebCanvasWorkspace`, `WebCanvasToolbar`, and `browser-canvas-runtime.ts` instead of desktop `CanvasPanel`, `CanvasToolbar`, `BottomPanel`, `DisplayLegend`, `CommandPalette`, command registry, desktop LayerPanel/CompassOverlay, or document-session lifecycle modules.
- Web Edition Species UI mounts through `WebSpeciesCatalogPanel`, not the desktop `PlantDbPanel`, `FavoritesPanel`, or full `PlantDetailCard`. Keep v1 controls limited to text search, climate zone, habit/growth form, life cycle, browser-local favorites, and recently viewed Species until the reduced web detail bead adds the narrow Plant Detail projection.
- Web Edition Location mounting lives under `desktop/web/src/web/`. Use `WebLocationWorkspace` and `app/location/coordinate-workbench.ts` instead of desktop geocoding/search Location modules, `LocationTab`, or `LocationInput`.
...
- Web Edition v1 Plant Detail may show one lazy-loaded remote hero image per Species when metadata is available. Show source/credit/license access with the image, tolerate missing or failed images cleanly, and do not assume the desktop native image cache exists in the web build.
...
- Web Edition v1 omits address search/geocoding. Location remains editable through manual latitude/longitude entry and map click or pin placement. See `docs/adr/0016-web-edition-omits-geocoding.md`.
- Web Edition Location UI should consume `app/location/coordinate-workbench.ts`; only desktop Location input/search surfaces should consume the search-capable `app/location/workbench.ts`.
...
- Web Edition v1 must not mount the Design Notebook. Use a simple browser-local Drafts list for reopen/resume flows only; do not add Notebook Sections, saved path references, file reveal actions, or notebook-style organization to the web build.
