...
- The notebook panel should use a command header plus a single searchable ledger, not a two-pane file-manager layout or a stack of unrelated strips. The header owns the title/count and compact icon actions such as Add Current Design and New Section. A compact control band owns search and Notebook View selection. The ledger below owns Notebook Section headers and dense rows. Follow `.interface-design/system.md`: parchment/ochre chrome, ruled separators, tokenized spacing/type, no green UI chrome, no nested card stacks.
- Notebook rows are navigation items: row click opens that Design; row handles are only for reordering or moving within notebook organization. Row actions for v1 should stay scoped to Pin/Unpin, Add/Move to Section, Remove from Notebook, and section rename/reorder/delete. Keep Pin/Unpin and Add/Move to Section visible because they are frequent organizing actions. Put only Remove from Notebook in a row overflow menu for now; it removes the notebook reference immediately without a modal and must not delete the `.canopi` Design file. Do not add Reveal in File Manager until a validated backend command exists.
