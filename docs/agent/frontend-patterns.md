...
- Browser-local Web Edition app data lives behind `desktop/web/src/web/browser-app-data.ts`. The compile-time browser settings adapter loads and saves the complete shared `Settings` contract through that store; `WebApp` owns only the browser Design Session autosave observer. Browser shell and Canvas settings commands mutate `app/settings/projection.ts` instead of receiving persistence callbacks or importing `localStorage`, file adapters, or app-data stores directly.
