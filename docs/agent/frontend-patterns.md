...
- Timeline Action Canvas behavior lives behind `app/timeline/canvas/`. `InteractiveTimeline` should import only that directory and consume `useTimelineActionCanvasHostModel()` plus the shared Canvas2D renderer hook. `app/timeline/canvas/host-model.ts` owns Planning Projection input assembly, layout offsets, canvas sizing, render callback/dependency assembly, event wiring, and tooltip/popover host-model projection. `app/timeline/canvas/controller.ts` owns Preact-facing interaction state, frame delegate assembly, render-state refs, and the small frame adapter seam. `app/timeline/canvas/interaction-frame.ts` owns canvas input ordering, selected Timeline Action identity, stale-selection cleanup, Target Presentation writes, popover open/save/delete orchestration, keyboard delete handling, and cleanup. Components should not import Timeline renderers, settings signals, interaction internals, controllers, or popover workbench helpers directly.
