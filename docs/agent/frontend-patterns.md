...
- Plant Symbol selection uses a separate toolbar popover after Plant Color. Show twelve labeled botanical choices in one four-column keyboard grid, followed by a separate Neutral dot choice. The neutral choice uses pressed state; the botanical grid uses selected options and retains a tab stop when neutral is selected. Arrow keys preview choices, while the two explicit apply actions distinguish selection from species default. Labels come from the shared symbol IDs and all eleven locales.
