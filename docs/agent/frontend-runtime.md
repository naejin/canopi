...
- `components/workspace/WorkspaceComposition.tsx` owns primary/side routing, SidePanelDock mounting, responsive planning-panel sizing, Calendar expansion reset on manual resize, lazy fallback placement, and shared workspace-dialog registration. It validates its supplied surface map against the shell panel projection, so command capabilities remain the panel-availability authority. `DesktopWorkspace.tsx` and `web/WebWorkspace.tsx` are concrete adapters; do not move native/browser I/O selection or another panel-availability list into the shared module.
...
- Web Edition v1 omits visible Location editing entirely. The desktop Location panel owns the search-capable Location Workbench and commits a pending search preview through the map-editing host. See `docs/adr/0005-web-edition-scope.md`.
...
- Planning panel width uses the shared side-dock setting. `WorkspaceComposition` gives Calendar expansion its temporary dock state; manual resizing exits expansion before `SidePanelDock` persists the normal width.
