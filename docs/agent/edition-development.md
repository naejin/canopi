...
| Canvas document | `SceneStore`, Canvas Runtime, replacement, runtime lease, and exhaustive cleanup mechanisms are shared | Desktop host sequencing stays in `use-canvas-document-session.ts` plus `document-session/lifecycle.ts`; Web sequencing stays in `WebCanvasWorkspace.tsx` plus `browser-design-session.ts`; see the [assessed lifecycle map](document-lifecycle.md#canvas-host-lifecycle-assessment) |
