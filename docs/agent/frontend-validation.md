...
- Canvas drawing code (rulers, Inspection Lens preview, PDF) receives `t` as a parameter; do not hardcode user-visible strings.
...
- Catalog boundary changes should pair Workbench lifecycle/adapter tests with both `favorites-panel.test.tsx` and `web-species-catalog-panel.test.tsx`. `favorite-species-presentation.test.ts` owns the shared accent-insensitive local match contract; the two component suites own their edition layout, actions, and focus restoration.
