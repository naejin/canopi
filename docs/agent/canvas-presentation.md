...
- Render Plant Symbols with native Pixi primitives (Canvas2D primitives in the Inspection Lens preview) rather than runtime SVG parsing, image textures, DOM overlays, or a new icon dependency. When the available radius is below 3.6 CSS px, collapse symbols to solid position dots without an outline. Retain explicit selection and hover cues.
