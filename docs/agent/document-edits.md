...
- Design Session owns the durable document-field history participant and the monotonic sequence shared with an attached Scene history. A document-field entry records only its field's before/after value; replay applies that field to the latest committed Design, so later edits to other fields remain intact. Canvas Undo/Redo chooses the greatest available sequence for undo and the smallest for redo. A new edit in either participant discards redo in both. Transient interaction history keeps priority. Do not add per-surface history controls or make SceneStore own document fields.
- Document-field history survives surface remounts because Design Session owns it. A Design replacement, dirty-baseline reset for an admitted successor, test fixture replacement, or Design Edit lifetime rollover aborts active preview and clears predecessor document history. An ordinary attached Canvas snapshot handoff must retain it.
