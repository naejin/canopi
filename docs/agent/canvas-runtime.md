...
- `canvas/runtime/scene-runtime/construction.ts` owns concrete runtime construction: SceneStore, CameraController, SceneHistory, RendererHost, presentation, document bridge, Scene Edit coordinator, mutation controller, and command/query/document role surface assembly. Keep new construction and disposal wiring behind that module instead of expanding `SceneCanvasRuntime`.
...
- Planning Canvas Interaction Frame (`app/planning-canvas/interaction-frame.ts`) owns shared planning-canvas document listener lifetime, active drag finish/abort ordering, hover/selection Target Presentation write ordering, stale visible-item cleanup, and disposal cleanup. It is not the Scene Interaction Frame; scene tools stay under `canvas/runtime/interaction/frame.ts`. Planning surface adapters keep geometry, rendering, local presentation state, and Design Edit semantics in their own modules while supplying drag lifecycle delegates to the frame.
