...
- `CanvasQuerySurface.getDesignObjectSelection()` is the app-facing read model for editable top-level Design Object selection, blocked selection reasons, and combined visual bounds. New selection actions, transform overlays, lock cues, and toolbar placement should consume this query instead of duplicating plant Visual Footprint, annotation bounds, Zone bounds, Layer, Object Group, or Design Object Lock rules.
