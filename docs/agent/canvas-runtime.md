...
- Timeline Action document editing belongs in `app/timeline/editing.ts`; Timeline drag math belongs in `app/timeline/interaction.ts`; Timeline Action Canvas behavior belongs behind `app/timeline/canvas/`. `host-model.ts` owns Planning Projection input assembly, layout offsets, canvas sizing, render callback/dependency assembly, event wiring, and tooltip/popover host-model projection. `controller.ts` owns Preact-facing interaction state, frame delegate assembly, render-state refs, and the small frame adapter seam. `interaction-frame.ts` owns pan, move, resize, autoscroll, origin freeze/restore, commit/abort, pointer ordering, wheel handling, popover open/save/delete/cancel ordering, keyboard delete, stale identity cleanup, Target Presentation writes, and drag cleanup. Canvas2D timeline UI consumes `useTimelineActionCanvasHostModel()` from `app/timeline/canvas/` plus the shared Canvas2D renderer hook; it must not import Timeline renderers, settings signals, interaction ordering, planning input assembly, or document edit transaction internals directly.
