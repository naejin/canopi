...
`scripts/pdf-native-webview/run.py` and its platform hosts run only the built production validation fixture supplied with explicit `--fixture` and `--script` paths. The [production probe workflow](../../.github/workflows/pdf-production-probe.yml) owns the four-platform invocation. Keep this harness production-facing; rejected encoder experiments and their dependencies do not belong in it.
