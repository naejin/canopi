...
Both paper sizes retain the approved 42 mm legend column, 10 mm margins, 10 pt legend text, 3 mm nominal legend symbols and 0.25 mm geometry strokes. Canvas marks are capped by 42% of nearest distinct planting spacing, with a 0.35 mm radius floor; all marks below 0.8 mm radius become solid position dots. Botanical silhouettes retain authored colours and use the same compact/detailed paths as the canvas; detailed geometry begins at a 12pt diameter (16 CSS px). Print capture includes both forms independently of the current viewport. Paths use nonzero winding so enclosed cutouts survive SVG preview and PDF encoding. A setup is limited to 200 total pages including overview and continuations. The job deadline is 120 seconds, font/name waits are bounded at 30 seconds, and native PDF delivery admits at most 64 MiB. These are engineering bounds, not performance promises for every device.
...
## Species key on paper

Print capture includes each Species’ reserved Design code. Legend entries retain full names and every appearance, prefixed by the same code shown in Species Key. Detail sheets place optional codes only where the label collision check clears authored text, plant marks, and page edges. These labels do not create coverage obligations, replace pinned names, inherit Species Focus, or change saved Design state. `app/canvas-pdf/species-codes.ts` owns this optional detail pass.
