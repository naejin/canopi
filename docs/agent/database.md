...
- `web/duckdb-wasm-catalog.ts` lazily creates one DuckDB database and connection per catalog reader and reuses them across searches, filter reads, and detail hydration. Catalog asset-registration failures after connection creation close both resources; do not replace the normal query path with per-request database setup and teardown.
- The reader lifecycle must close the connection and terminate the database when the live catalog is disposed, including Vite HMR teardown.
- The reader registers Species assets at startup, active-locale Common Name assets on demand, and image metadata on first detail-image read. Keep inactive locales, image metadata, and full Species rows out of eager JavaScript startup state.
...
- Web catalog failures surface through Workbench search or detail error state; search failures with no retained results expose Retry. Do not silently return an empty catalog or hide the Plant Database panel when catalog assets fail.
