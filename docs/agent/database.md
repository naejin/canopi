...
- Reject a `user_version` newer than the supported registry instead of attempting to read or mutate it. Keep every historical registry position explicit, including the intentional v6 no-op left after the short-lived pinned Notebook field was retired.
- Migration SQL and its `user_version` advance must share one transaction. Schema rebuild migrations must preserve valid dependent rows while foreign keys remain enabled; do not toggle `PRAGMA foreign_keys` inside migration SQL.
