...
| Pixi text raster density | Twice the renderer DPR, independent of camera zoom | Supersample only text textures to preserve glyph edges at fractional screen positions and rotation. Font family, screen size, and smooth positioning remain unchanged. Text textures use about four times the pixel storage; the canvas backing store stays at normal device density. |
...
The acceptance run discovered a capability probe defect (`canopi-o1me`, now fixed): `detectRendererCapabilities` requested incompatible context types on the same canvas, causing false WebGL negatives after acquiring 2D. That explains the Canvas2D selection in these historical captures. Production now probes each context type on an independent disposable canvas, and regression tests cover independent DOM/OffscreenCanvas probes and automatic backend selection. For backend verification, fresh independent canvases confirmed WebGL/WebGL2 support, and those measured capability facts were supplied to the existing RendererHost. It initialized actual Pixi, handled a deliberately injected render failure by switching to actual Canvas2D, rendered the same Scene, accepted a viewport-only change to 14 px/m, retained exactly one canvas, and removed it on disposal. This is an injected failure test, not a claim of physical GPU failure testing. The direct paired Pixi captures above do not depend on the faulty automatic probe.
