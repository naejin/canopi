...
| design location tools | Shipped — the visible PanelBar entry opens the dedicated location flow while the canvas consumes the saved origin through the derived basemap; see `docs/todo.md` MapLibre / geo for current implementation detail | shipped | Wave 3 | Let users set the design location from a dedicated map flow with search, drag, zoom, and centered-pin confirmation | `LocationTab` opens a map picker with overlay search, a fixed center pin, standard drag/zoom behavior, and a confirm flow that updates the in-canvas basemap origin |
...
| geo / terrain map features | In progress — the current shipped slice is tracked in `docs/todo.md` (`MapLibre / geo`), while richer terrain/offline layers remain deferred | in progress | Post-convergence | Advanced geo and terrain context for designs | Basemap stays non-authoritative and camera-following; rendered overlays consume the pure map projection seam without creating a second authority |
...
5. Use the dedicated location flow to search, drag, zoom, and confirm the design location, then see the canvas basemap follow that saved origin.
