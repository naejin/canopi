...
| design location tools | Hidden in PanelBar, but the dedicated location flow remains live and the canvas now consumes saved location via the in-canvas basemap | shipped (hidden) | Wave 3 | Let users set the design location from a dedicated map flow with search, drag, zoom, and centered-pin confirmation | `LocationTab` opens a map picker with overlay search, a fixed center pin, standard drag/zoom behavior, and a confirm flow that updates the in-canvas basemap origin |
...
| geo / terrain map features | Initial in-canvas basemap slice shipped; overlays, contours/hillshade, offline tiles, and richer geo layers remain deferred | in progress | Post-convergence | Advanced geo and terrain context for designs | Basemap stays non-interactive and camera-following; future overlays consume the pure map projection seam without creating a second authority |
