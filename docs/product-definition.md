...
| design location tools | Active | shipped | Wave 3 | Let users set the design location from a bottom-bar map tab with search, drag, zoom, and centered-pin confirmation | Bottom-bar `location` tab opens a map picker with overlay search, a fixed center pin, standard drag/zoom behavior, and a confirm flow that updates canvas map layers |
| timeline tools | Deferred; internal tab plumbing exists but hidden. Requires identity semantics convergence (see architecture review Finding 2, `docs/todo.md`) | deferred | Post-convergence | Plan time-based work and scheduling inside a design | Panel identity semantics defined; document authority converged |
| budget tools | Deferred; internal tab plumbing exists but hidden. Requires identity semantics convergence (see architecture review Finding 2, `docs/todo.md`) | deferred | Post-convergence | Track design costs and budget outputs | Panel identity semantics defined; document authority converged |
...
| consortium editing surface | Deferred; internal plumbing exists. Uses placed-plant IDs (correct identity scheme). Requires document authority convergence | deferred | Post-convergence | Restore consortium workflows as a bottom-bar tab | Document authority converged; panel↔canvas sync working |
