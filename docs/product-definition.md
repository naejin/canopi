...
| geo / terrain map features | Initial in-canvas basemap slice shipped; basemap loading/error feedback plus panel↔map hover/selection overlays are live, while contours/hillshade, offline tiles, and richer geo layers remain deferred | in progress | Post-convergence | Advanced geo and terrain context for designs | Basemap stays non-authoritative and camera-following; rendered overlays consume the pure map projection seam without creating a second authority |
