...
The Web Edition v1 should use a browser-specific app shell around the shared Canopi app core instead of mounting desktop-native window controls or native file-management actions. Its chrome should still match the desktop workspace UX as closely as practical: desktop-style top bar structure, menu/dropdown interaction, centered Design identity, and the right-side PanelBar pattern. The Browser App Shell should expose the v1 web command set: New, Open `.canopi`, Download `.canopi`, language/theme, and panel toggles. Browser Drafts remain internal autosave/recovery state rather than a visible shell command or list.
