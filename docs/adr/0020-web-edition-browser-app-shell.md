...
The implemented shells share `components/workspace/WorkspaceComposition.tsx` for command-to-surface validation, primary/side routing, dock policy, planning-panel sizing, and workspace dialog placement. `DesktopWorkspace.tsx` and `web/WebWorkspace.tsx` remain concrete surface adapters so the browser shell cannot acquire native panels through the shared composition.
