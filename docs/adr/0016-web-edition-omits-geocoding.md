...
The Web Edition v1 keeps Location editing through manual latitude/longitude entry and map click, but does not include address search, geocoding, or altitude editing. Web coordinate and map-pick commits store `altitude_m` as `null` for the changed Location.
