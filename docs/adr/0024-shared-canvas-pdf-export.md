...
This records an approved direction; the shared Canvas PDF feature is not implemented yet. The initial default scale remains unresolved. Printed examples must validate text and symbol readability and inform the default scale and column widths. The PDF library, vector/raster strategy, paper defaults, margins, overlap size, Layer and detail-selection defaults, Zone context padding, handling of authored plant names, exceptional legend overflow behavior, and map inclusion also remain unresolved. Any map inclusion must account for the existing [Web map scope](0013-web-edition-map-scope.md), attribution, and provider constraints; local PDF generation does not imply an [offline-first Web app](0022-web-edition-not-offline-first.md).
