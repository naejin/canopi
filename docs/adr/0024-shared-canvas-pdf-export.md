...
The first release supports A4 and US Letter paper. Detail pages support a chosen real-world scale, such as 1:100 or 1:200, so users can measure Design distances with a ruler when printing at actual size. The overview fits its page, and every page includes a scale bar.
...
This records an approved direction; the shared Canvas PDF feature is not implemented yet. The PDF library, vector/raster strategy, paper defaults, orientation, margins, label layout, and map inclusion remain unresolved. Any map inclusion must account for the existing [Web map scope](0013-web-edition-map-scope.md), attribution, and provider constraints; local PDF generation does not imply an [offline-first Web app](0022-web-edition-not-offline-first.md).
