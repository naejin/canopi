...
The default overview covers the full extent of the Design, including areas outside the current view. Canvas pan and zoom do not determine the exported area. Users choose which Zones receive detail pages, and each detail page focuses on one chosen Zone. Zone selection controls detail coverage while the overview retains the whole-Design context.
...
This records an approved direction; the shared Canvas PDF feature is not implemented yet. The PDF library, vector/raster strategy, paper defaults, margins, Layer selection defaults, Zone framing and content, oversized-Zone and no-Zone behavior, optional free-area selection, label/legend layout, and map inclusion remain unresolved. Any map inclusion must account for the existing [Web map scope](0013-web-edition-map-scope.md), attribution, and provider constraints; local PDF generation does not imply an [offline-first Web app](0022-web-edition-not-offline-first.md).
