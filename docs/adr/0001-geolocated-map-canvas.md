...
- **Map layers.** The default basemap is the OpenFreeMap Liberty vector style (Positron, Bright, Dark selectable) with OpenFreeMap/OpenMapTiles/OpenStreetMap attribution. Satellite is its own Site references row under Basemap and hides the Basemap when on; providers are Esri World Imagery (keyless, after a terms check) and Google only with a device key. OSM raster tiles and MapTiler are removed.
