...
The built Web Edition should move to `canopi-website` as a versioned build artifact during deployment. The production artifact source is a public Canopi app release asset produced by CI and attached to the matching release tag. The artifact should include a small manifest with the app version, git commit, base path, asset list, and SHA-256 checksums so the website deploy can locate and verify exactly what it publishes. Release retention follows normal release retention; transient CI artifacts are useful for debugging but are not the website deploy source of truth.

The website repository should not commit generated `/app` assets long term. Local development may use a script that copies the Web Edition build from a sibling Canopi checkout into the website output tree, but that copy is a local/dev handoff, not the source-of-truth distribution model.
