...
- Every copied file keeps an MIT header with its source path and commit and gets an entry in `THIRD_PARTY_NOTICES`.
...
- Upstream fixes reach Canopi through dependency bumps or deliberate re-copies at a new commit, recorded in `THIRD_PARTY_NOTICES`.
