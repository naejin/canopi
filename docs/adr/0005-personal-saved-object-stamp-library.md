...
**Consequences**:
Desktop stamp import/export must not route through Design Session open/save flows, must not update Recent Designs, and must not include design-level state such as Location, Budget, Timeline, Consortiums, or hidden object metadata. Web Saved Object Stamp import/export requires a decision explicitly authorizing stamp portability. The Canvas PDF scope in [ADR 0024](0024-shared-canvas-pdf-export.md) leaves this restriction in force.
