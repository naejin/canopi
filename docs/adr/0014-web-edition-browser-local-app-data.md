...
This accepts same-resource last-writer-wins concurrency and occasional legacy aggregate work during transition or quota recovery. Storage keys, staged migration, quota handling, and finalization belong behind the store, not in shared UI. The [Browser App Data guide](../agent/browser-edition.md#browser-app-data) records those implementation rules.
