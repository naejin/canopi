# Export a Canvas PDF

Open a Design and choose **File → Export to PDF**. The print workspace shows your pages before you save them. **Save PDF**, with the total page count, stays at the top: desktop opens a PDF save dialog and Web Edition requests a download. Keep the .canopi file to edit your Design later.

Choose A4 or US Letter and open **Print layers** to choose the Design Layers to print. Initially, export uses the visible printable layers. These choices never change your Design's visibility or content. Maps and the Timeline, Budget, and Consortium tabs are excluded.

Start with the overview. Choose **Add page**, then click a Zone or drag a rectangle on the fitted overview. You can also search the Zone list and select a result; Enter selects the only matching result. The new detail page opens with the whole area fitted and the best orientation selected automatically. Exact squares use portrait. Each requested area produces one canvas page. Drawn Print Areas remain temporary and never create Zones.

The Add page overview fits all available Zones independently of the printed overview's zoom and position. Cancelling returns to the previous page without changing its framing. Zone outlines in this chooser are selection guides; printing them still depends on Print layers.

Click a page thumbnail to open it. On the printed overview, click a numbered coverage outline to open its detail page. Hover or focus a thumbnail to highlight its coverage on the overview. Remove a detail page with its × button; its associated legend pages are removed too.

Drag the drawing within the paper to adjust its printed position. With the paper focused, arrow keys also move the drawing; Shift moves it farther. Adjust **Canvas zoom (%)** directly or use **− / +**. Lower values show more surroundings and higher values enlarge the drawing. Manual movement or zoom can crop the selected area. **Fit** restores both the original centre and 100% zoom so the whole area is visible again.

**Automatic / Portrait / Landscape** applies to the selected page only, including legend pages. Changing orientation retains that page's zoom and manual displacement. **Inspect text** temporarily enlarges the paper on screen; scroll to read it, then choose **Done inspecting** or press Escape. Inspection never changes the printed scale.

Detail pages have a narrow plant legend covering the plants visible in their current framing, using the current language's common name when available, otherwise the full botanical name. An overview used alone also has a legend; with details, it becomes a navigation sheet. Plant marks adapt to nearby spacing on paper while retaining their authored symbols and colours. If several species share an appearance, the preview warns you; return to the Design to edit it or export as shown.

The overview moves annotations, pinned plant names and distances to detail pages only when they fit there completely without collisions. When text has no readable detail home, choose **Add detail page** or **Keep text on overview** before saving. Full text remains visible while you decide. Moving or removing a detail page rechecks coverage; changing previously retained text or its placement may require a new choice. These print choices never edit your Design.

When a plant list needs more room, choose **Add legend pages**. The linked pages appear under their source thumbnail and preserve complete names and appearances. Export stays unavailable while a legend overflows. **Remove legend pages** withdraws that consent for the current setup; adjust the framing or accept continuations again before exporting if they are still needed.

Choose **Back to design** to edit, then reopen Export to PDF to keep the current session's print choices. Open previews refresh automatically when source content changes. A renamed or removed selected Zone needs explicit review: remove its retained page and add the intended Zone. Opening another Design discards the old print setup. Generating or saving a PDF never changes the Design or marks it saved.

Print at **100% / actual size**. Disable the printer dialog's fit-to-page scaling. The calibration bar measures 50 mm on every canvas page; its distance label states the corresponding ground distance. Each page has its own fitted physical scale; the displayed ratio is approximate. Canvas zoom and screen inspection are separate from the printer's 100% setting. Physical defaults still await the recorded [paper and field review](canvas-pdf-validation.md).
