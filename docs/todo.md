...
- ~~In-canvas MapLibre base layer behind the canvas~~ — **done**: `CanvasPanel` mounts `MapLibreCanvasSurface` behind `.canvasContainer`; the app-level map surface lifecycle lazy-loads `maplibre-gl`, stays non-interactive, and reports loading / ready / error state back to the canvas shell
...
- ~~Bearing-aware canvas↔map sync seam~~ — **done**: `canvas/projection.ts` / `canvas/maplibre-camera.ts` now own the shared Mercator-backed local projection frame through an explicit active projection-backend seam, `north_bearing_deg` participates in both camera derivation and map-feature projection, and in-canvas pan/zoom sync no longer relies on separate bearing math or 256px-world zoom shortcuts in the map surface
...
- ~~Precision warning for large local-projection extents~~ — **done**: the canvas map feedback now warns once design geometry extends beyond the local precision threshold; the metric is backend-derived metadata surfaced by the map state adapter, the map stays active, and diagnostics remain dev-only
...
- ~~Map surface state/overlay/terrain helpers extracted from `MapLibreCanvasSurface`~~ — **done**: state/diagnostic shaping, basemap presentation/bootstrap helpers, overlay coordination, and terrain diff/apply logic now live in dedicated helper modules
- ~~Imperative in-canvas map surface lifecycle extracted from Preact~~ — **done**: `app/canvas-map-surface/lifecycle.ts` owns MapLibre construction, async generation cancellation, camera application through the projection seam, terrain sync, overlays, diagnostics, and resize cleanup; `MapLibreCanvasSurface` now only mounts the lifecycle and forwards signal snapshots
...
- Local-projection backend replacement for larger geodetic use cases — deferred: the current active backend is still the Mercator-backed local frame, with warning-only guardrails for large designs
...
- ~~Timeline UX rework~~ — **done**: replaced species-grouped rows with 6 fixed action-type rows (always visible), removed header/form, click-to-add/edit floating popover with date validation (min/max constraints + inline error), edge resize, hover tooltip, species color priority for bar colors, ctrl+scroll zoom (0.2-20 pxPerDay range with cursor-anchored scaling). Ruler controls hidden pending design iteration. Tab re-enabled in `VISIBLE_BOTTOM_PANEL_TABS`
