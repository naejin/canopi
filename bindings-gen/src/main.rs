...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let design_format_output = root.join("../desktop/web/src/generated/canopi-design-format.ts");
    let settings_output = root.join("../desktop/web/src/generated/settings.ts");
    ...
    write_canopi_design_format(&design_format_output)?;
    write_settings_runtime(&settings_output)?;
...
fn write_canopi_design_format(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn write_settings_runtime(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
#[cfg(test)]
mod tests {
    use super::{python_command_candidates, validate_canopi_design_schema};
    use std::ffi::OsString;
