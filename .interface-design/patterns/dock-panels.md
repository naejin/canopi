...
The Layers dock ends with a LiDAR section (`components/panels/lidar/LidarLayersSection.tsx`) that follows the compact ruled-row pattern: a quiet group heading with one Add-layer command, then source-layer rows with eye toggle, name, measurement-kind chip, state chip and inline actions (Add TIFFs, Analyse, Delete with impact confirmation). Analysis results indent below their source with their own eye, state and opacity; hiding or collapsing a source never changes result visibility. Opacity is a plain range input per row. The import review is a bordered block that appears inside the section while a staged import awaits review: source list with issues, exact uncovered/overlap/invalid counts, fixed-style Before/After previews, the two independent region decisions and Apply/Cancel (Cancel and Escape publish nothing). Use `canvas.lidar.*` i18n keys in all locales; tokens only — no raw colors, radii, font sizes or spacing values.
