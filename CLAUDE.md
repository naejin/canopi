...
- **Plant detail query has one projection owner**: `detail.rs` owns a single ordered species-detail projection and row cursor. When adding/removing plant-detail fields, update that projection owner, the mapper, and the contract tests in the same patch instead of editing scattered `row.get(...)` sites
...
- **Schema version**: `PRAGMA user_version = 5` in canopi-core.db. Rust backend warns if outside the shared expected version at startup. Export schema version 8 (`min_export_schema_version` in contract). Contract version 5 = 176 species columns
