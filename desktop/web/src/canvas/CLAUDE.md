...
- **`Date.now()` / `Infinity` in layout sort fallbacks**: Dateless timeline actions use `Infinity` (sort last, stable) not `Date.now()` (non-deterministic, defeats `useMemo`). See `computeLayout` in `timeline-renderer.ts`
