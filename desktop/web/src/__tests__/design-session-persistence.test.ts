...
describe('Design Session persistence', () => {
  ...
  it('snapshots attached canvas content into the current Design Session', () => {
    ...
    const session = makeSession((metadata, doc) => ({
      ...doc,
      name: metadata.name,
      zones: [{ name: 'canvas-zone', zone_type: 'bed', rotation: 0, points: [], fill_color: null, notes: null, locked: false }],
    }))
    store.replaceCurrentDesignState(design, null, design.name)
    ...
    expect(result?.zones).toEqual([{ name: 'canvas-zone', zone_type: 'bed', rotation: 0, points: [], fill_color: null, notes: null, locked: false }])
