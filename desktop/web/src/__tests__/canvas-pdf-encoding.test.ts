...
it('encodes the shared mixed-script overview as vector PDF bytes with embedded fonts', async () => {
  ...
  const plan = buildPdfPlan(input, { paper: 'Letter', orientation: 'landscape', layers: ['annotations'] }, createPdfTextEngine(fonts, 'ja'),
    { overview: 'Overview', plants: 'Plants', actualSize: 'Print at actual size', page: 'Page' })
...
it('loads every regional font needed when mixed-script names wrap onto different lines', async () => {
  ...
  try {
    const result = await preparePdf({
      ...
      labels: { overview: 'Overview', plants: 'Plants', actualSize: 'Print at actual size', page: 'Page' }, fontBaseUrl: 'https://app.test/fonts/',
    })
