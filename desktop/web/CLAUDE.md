...
- **Dropdown is viewport-aware**: `Dropdown.tsx` auto-flips direction and clamps `max-height` to available viewport space when opened near screen edges. Prefer `menuDirection="down"` (default) — the component handles edge cases automatically. Use `"up"` only when the trigger is anchored to the very bottom of the viewport (e.g., canvas bar)
