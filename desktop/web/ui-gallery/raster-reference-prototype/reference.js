...
(() => {
  ...
  function reviewBar() {
    return `<div class="review"><a href="index.html" data-nav="index" class="reference-tag">CANOPI / UI REFERENCE · APPROVED · 0e696722</a><div class="grow"></div>${page !== 'index' ? `<label>State<select id="scenario" aria-label="Reference state">${pages[page].states.map(s => `<option ${s === state ? 'selected' : ''}>${s}</option>`).join('')}</select></label>` : ''}<label>Edition<select id="edition"><option value="desktop" ${edition === 'desktop' ? 'selected' : ''}>Desktop</option><option value="web" ${edition === 'web' ? 'selected' : ''}>Web</option></select></label>${btn(theme === 'dark' ? 'Light theme' : 'Dark theme', 'theme')}${btn(narrow ? 'Full width' : 'Narrow width', 'width')}${btn('Reset', 'reset')}</div>`;
  }
  function terrain() {
