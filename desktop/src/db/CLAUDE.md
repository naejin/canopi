...
- **`species_climate_zones` junction table (v11 export, schema v8)**: Climate zone assignments per species (species_id, climate_zone, confidence, source). 311K rows, 7 zones (Tropical, Arid, Mediterranean, Subtropical, Temperate, Continental, Boreal). Filtered via EXISTS subquery in `append_structured_filters`. `climate_zones` JSON column on species table stores the same data for detail display
- **FilterStrip filter migration (schema v8)**: `stratum`, `hardiness_zone_min/max`, `height_max_m` moved from dedicated `SpeciesFilter` fields to the dynamic filter path (`validated_column()` + `field-registry.ts`). Replaced in FilterStrip by `climate_zones` (chips), `growth_form_type` (chips), and `woody` (toggle)
