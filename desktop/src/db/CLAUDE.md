...
- **Schema version**: `PRAGMA user_version = 7` in canopi-core.db. Rust backend warns if outside the shared expected version at startup. Export schema version 10 (`min_export_schema_version` in contract). Version is validated at startup via `schema_contract.rs`
