...
- **Filter UI architecture**: Always-visible filters in `FilterStrip.tsx` use typed `SpeciesFilter` fields. "More filters" panel uses dynamic `Vec<DynamicFilter>` channel with the generated Rust `validated_column()` allowlist. Add dynamic, strip, hidden, kind, category, and ordering metadata in `common-types/plant-filter-fields.json`, then run `cd desktop/web && npm run gen:types` to refresh the TS and Rust adapters. `patchFilters()` in `app/plant-browser/controller.ts` is the single mutation point for filter state. `dynamicOptionsCache` is locale-scoped (`Record<locale, Record<field, DynamicFilterOptions>>`); `dynamicOptionsPending` deduplicates concurrent IPC requests per locale+field
...
- **FilterStrip <-> More Filters exclusion**: Strip-only fields are declared with `ui_placement: "strip"` in `common-types/plant-filter-fields.json`. More Filters consumes `dynamicFilterFieldsForCategory()` from the generated TS adapter, so strip fields such as `habit`, `woody`, and `climate_zones` do not need a separate exclusion set.
...
- **Dynamic filter option ordering**: `value-ordering.ts` exports `orderFilterValues(field, values)` which sorts dynamic filter options semantically from ordering metadata generated out of `common-types/plant-filter-fields.json`. Fields without an ordering keep alphabetical DB order.
...
## MapLibre / Terrain
Current MapLibre usage is frontend-owned in the full-screen `LocationTab`, featured-design `WorldMapSurface`, and the in-canvas `MapLibreCanvasSurface` basemap. All live surfaces share `DEFAULT_MAPLIBRE_BASEMAP_STYLE_URL` from `maplibre/config.ts`, so hosted basemap changes stay in sync. The current in-canvas slice is derived, non-authoritative, lazy-loaded, and non-interactive for direct map gestures, but it does emit loading / ready / error feedback and renders panel-target hover / selection overlays through the active projection backend seam. These gotchas apply when extending beyond that slice:
...
- **Precision warnings are projection-backend outputs**: the `10 km` warning threshold and dev diagnostics come from the active projection seam; UI modules should surface that metadata, not recompute it
