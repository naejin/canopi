...
- **FilterStrip is data-driven**: Chip-row filters are defined as a `chipRows` config array in `FilterStrip.tsx`, not individual JSX blocks. Add new chip filters by adding an entry to the array. Toggle filters (woody, nitrogen) remain inline JSX
- **FilterStrip <-> More Filters exclusion**: Fields that are first-class `SpeciesFilter` typed fields AND exist in `field-registry.ts` must be listed in `STRIP_FIELDS` in `MoreFiltersPanel.tsx` to prevent double-filtering. Currently: `growth_form_type`, `woody`
