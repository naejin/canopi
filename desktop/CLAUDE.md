...
- **SearchBar count badge uses `totalEstimate` from backend `total_estimate`**: `build_count_query` in Rust computes the real count. SearchBar shows it when `> 0`. Count and clear button are both absolutely positioned in `.searchWrap` — `.resultCountShifted` offsets the count to clear the button using tokenized calc
